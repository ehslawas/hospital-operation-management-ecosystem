...
export default async function InventoryPage() {
  const items = getItems().sort((a, b) => a.name.localeCompare(b.name));
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">Complete pharmaceutical and medical supply inventory</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 p-6 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <div className="relative">
              <div className="text-sm font-semibold text-blue-800 mb-2">Total Batches</div>
              <div className="text-3xl font-bold text-blue-900">{batches.length}</div>
              <div className="text-xs text-blue-700 mt-1">All inventory batches</div>
            </div>
          </div>
        ...
        <InventoryTable 
          ...
          title="Stock List"
          description="Complete inventory overview with batch tracking"
        />
      </div>
    </div>
  );
