...
export default async function DrugInventoryPage() {
  const allItems = getItems().filter(item => item.category === 'Drug').sort((a, b) => a.name.localeCompare(b.name));
  const batches = getBatches();
  const locations = getLocations();
  
  // For now, we'll show all items. In a real app, this would come from search/filter params
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="mx-auto max-w-7xl space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Drug Inventory</h1>
            <p className="mt-2 text-slate-600">Manage pharmaceutical inventory with precision</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 p-6 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <div className="relative">
              <div className="text-sm font-semibold text-blue-800 mb-2">Drug Batches</div>
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 p-6 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <div className="relative">
              <div className="text-sm font-semibold text-emerald-800 mb-2">Drug Types</div>
              <div className="text-3xl font-bold text-emerald-900">{items.length}</div>
              <div className="text-xs text-emerald-700 mt-1">Unique drugs</div>
            </div>
          </div>
        </div>
        ...
        <InventoryTable 
          items={items}
          batches={batches}
          placeholder="Search drug name, code, batch..."
          title="Drug Stock List"
          description="Comprehensive view of all pharmaceutical inventory"
        />
      </div>
    </div>
  );
