...
async function main() {
  ...
  for (const idx of stepsToUndo) {
    ...
    if (idx === 145) {
      // Modify line 221 of Step 145 replacement to have the extra closing div
      for (let j = 0; j < replacement.length; j++) {
        if (replacement[j].trim() === '</div>' && j === 221) {
          console.log("Modifying Step 145 replacement at relative index", j);
    ...
    if (startIdx === -1) {
      ...
      if (idx === 145) {
        ...
        console.log("Matches for first line of Step 145:", matches);
