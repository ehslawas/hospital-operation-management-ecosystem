...
function cleanClassAttr(str) {
  return str.replace(/className="[^"]*"/g, '').replace(/class="[^"]*"/g, '').trim();
}
...
function findHunkRange(fileLines, repLines) {
  ...
  for (let i = 0; i <= fileLines.length - repLines.length; i++) {
    ...
    for (let j = 0; j < repLines.length; j++) {
      const fl = cleanClassAttr(fileLines[i + j]);
      const rl = cleanClassAttr(repLines[j]);
      if (fl !== rl) {
        match = false;
        break;
      }
...
async function main() {
  ...
  for (const idx of stepsToUndo) {
    if (!stepMap[idx]) {
      console.error(`ERROR: Could not find step ${idx}`);
      return;
    }
  }
  ...
  for (const idx of stepsToUndo) {
    ...
    // Memory patches for overlapping / leftover edits during step-by-step rollback
    if (idx === 235) {
    ...
    if (idx === 85) {
      replacement.splice(12, 2);
      const len = replacement.length;
      replacement[len - 1] = "          </div>         </div>";
    }

    const startIdx = findHunkRange(fileLines, replacement);
    if (startIdx === -1) {
      console.error(`  ERROR: Could not find hunk for step ${idx}!`);
      return;
    }

    fileLines.splice(startIdx, replacement.length, ...target);
    console.log(`  Successfully reverted step ${idx}. Range: lines ${startIdx + 1} to ${startIdx + replacement.length + 1}`);
  }

  // Write back to file with CRLF
  fs.writeFileSync(targetFilePath, fileLines.join('\r\n'), 'utf8');
  console.log("SUCCESS! OxygenDashboardPage.tsx successfully rolled back to pre-cutoff state!");
}
