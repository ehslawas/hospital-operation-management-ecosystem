...
for (const line of transcriptLines) {
  try {
    ...
    if (step.step_index === 37) {
      console.log("=== STEP 37 TARGET LINES ===");
      const target = step.tool_calls[0].args.TargetContent.split('\n');
      for (let i = 0; i < Math.min(5, target.length); i++) {
        console.log(`Line ${i}: [${target[i]}]`);
      }
    }
  } catch (e) {}
}
