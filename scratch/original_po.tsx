...
export const PurchaseOrderDetailPage: React.FC = () => {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      console.log(`Γ£ô Successfully found ${allPages.length} .page element(s) for PDF conversion`)
              ...
              <td className="w-1/2 align-top pr-4" style={{ width: '50%', verticalAlign: 'top', paddingRight: '1rem' }}>
                <div className="space-y-2">
                  ...
                  <div className="border-b border-gray-400 pb-1">
                    ...
                    <p className="text-sm font-semibold text-gray-900">{order.vote_code || 'ΓÇö'}</p>
                  </div>
                  <div className="border-b border-gray-400 pb-1">
                    <label className="text-xs font-bold text-gray-600 uppercase block mb-0.5">Aktiviti Undi / Vote Activity</label>
                    <p className="text-sm font-semibold text-gray-900">{order.vote_activity || 'ΓÇö'}</p>
                  </div>
              ...
              <td className="w-1/2 align-top pl-4" style={{ width: '50%', verticalAlign: 'top', paddingLeft: '1rem' }}>
                <div className="space-y-2">
                  <div className="border-b border-gray-400 pb-1">
                    ...
                    <p className="text-sm font-semibold text-gray-900 uppercase">{order.department || 'ΓÇö'}</p>
                  </div>
                  <div className="border-b border-gray-400 pb-1">
                  ...
                  <div className="border-b border-gray-400 pb-1">
                    ...
                    <p className="text-sm font-semibold text-gray-900 uppercase">{order.category?.replace('_', ' ') || 'ΓÇö'}</p>
                  </div>
        ...
        <div className="grid grid-cols-1 gap-2">
          <div className="border border-gray-600 p-2 bg-white">
            ...
            {order.po_type === 'sq' && order.sq_suppliers && order.sq_suppliers.length > 0 ? (
              ...
              <p className="text-sm font-semibold text-gray-900 uppercase">{order.supplier?.company_name || order.manual_supplier_name || 'ΓÇö'}</p>
            )}
          </div>
                  ...
                  return (
                    <tr key={item.id} className={`${isEditing ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className="border border-gray-600 px-2 py-0.5 text-xs text-gray-900">
                        <span className="font-bold">{item.item_name || 'ΓÇö'}</span>
                        {/* SQ Details */}
                        {order.po_type === 'sq' && (
                      ...
                      <td className="border border-gray-600 px-2 py-1 text-xs text-gray-700 font-mono">{item.item_code || 'ΓÇö'}</td>
                      ...
                      <td className="border border-gray-600 px-2 py-1 text-xs text-gray-700">
                        {isEditing ? (
                          ...
                          item.packaging_description || 'ΓÇö'
                        )}
                      </td>
                    </tr>
                  )
              ...
              <tbody>
                <tr className="border-b border-gray-800">
                  ...
                  <td className="px-2 py-1.5 text-[10.5pt] font-bold text-right" style={{ whiteSpace: 'nowrap' }}>
                    {balance !== null ? formatCurrencyMalay(balance) : 'ΓÇö'}
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td className="px-2 py-1.5 text-[11.5pt] font-black text-right" style={{ whiteSpace: 'nowrap' }}>
                    {(!order.vote_code || order.vote_code === '-') && (!order.vote_activity || order.vote_activity === '-') ? formatCurrencyMalay(0) : (balance !== null ? formatCurrencyMalay(balance - total) : 'ΓÇö')}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <table className="w-full max-w-4xl border-collapse border-2 border-gray-800">
            <tbody>
              <tr>
                ...
                <td className="border border-gray-800 px-3 py-1.5 font-bold text-sm uppercase" style={{ lineHeight: '1.3' }}>
                  {order.po_type === 'sq' && order.sq_suppliers && order.sq_suppliers.length > 0 ? (
                    ...
                    <>
                      {order.supplier?.company_name || order.manual_supplier_name || 'ΓÇö'}
                      <br />
              ...
              <tr>
                ...
                <td className="border border-gray-800 px-3 py-1.5 font-bold text-sm">{order.supplier?.phone || 'ΓÇö'}</td>
              </tr>
            </tbody>
          </table>
        </div>
