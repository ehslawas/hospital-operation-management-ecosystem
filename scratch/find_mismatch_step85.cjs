...
async function main() {
    ...
    if (startIdx === -1) {
      ...
      if (idx === 85) {
        ...
        for (let i = 0; i < fileLines.length; i++) {
          if (cleanClassAttr(fileLines[i]) === cleanClassAttr(replacement[0])) {
            matches.push(i);
          }
        }
        console.log("Matches for first line of Step 85:", matches);
        for (const m of matches) {
          console.log(`Checking match starting at line ${m + 1}`);
          for (let j = 0; j < replacement.length; j++) {
            if (m + j >= fileLines.length) {
              console.log(`  EOF reached at j=${j}`);
              break;
            }
            const fl = cleanClassAttr(fileLines[m + j]);
            const rl = cleanClassAttr(replacement[j]);
            if (fl !== rl) {
  ...
  console.log("ALL STEPS REVERTED IN MEMORY SUCCESSFULLY!");
}
