    ...
    fetchMenus: async (userId: string, options?: { roleCode?: string, departmentCode?: string, departmentName?: string, user?: any }) => {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                const isAdmin = isSystemAdmin || isHospitalAdmin
                ...
                allMenus?.forEach((m: any) => {
                    ...
                    if (isAdmin) {
                        // STRICT SEPARATION:
                        // System Admin -> sees 'system_admin' OR 'dashboard'
                        // Hospital Admin -> sees 'hospital_admin' OR 'dashboard'
                        if (isSystemAdmin) {
                            // System Admin sees 'system_admin', 'dashboard', and shared 'admin.*'
                            isAccessible = code === 'system_admin' || code === 'dashboard' || code.startsWith('admin.')
                        } else if (isHospitalAdmin) {
                            // Hospital Admin sees 'hospital_admin', 'dashboard', and shared 'admin.*'
                            isAccessible = code === 'hospital_admin' || code === 'dashboard' || code.startsWith('admin.')
                        }
                    }
