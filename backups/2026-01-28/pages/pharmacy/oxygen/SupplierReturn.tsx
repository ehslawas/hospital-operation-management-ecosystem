...
export const SupplierReturn: React.FC = () => {
    ...
    // Mock Data for "Recent Logistics Manifests"
    const MOCK_RECENT_RETURNS = [
        {
            id: 'REF-2024-001-LX',
            date: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString(), // 2 hours ago
            remarks: 'Returned to LINDE EOX SDN BHD (CAW. MIRI)',
            qty: 15,
            sizes: { 'J': 10, 'F': 5 },
            cylinders: Array.from({ length: 15 }).map((_, i) => ({
                qr_code: `OXY-RET-J-${100 + i}`,
                serial_number: `LX-SN-${9000 + i}`,
                size: { code: i < 10 ? 'J' : 'F' },
                ownership: i < 5 ? 'Private' : 'Vendor' // Mock mix of Private and Vendor
            }))
        },
        {
            id: 'REF-2024-002-LX',
            date: new Date(Date.now() - 1000 * 60 * 60 * 25).toISOString(), // Yesterday
            remarks: 'Defective Valve Replacement - Batch A2',
            qty: 4,
            sizes: { 'E': 4 },
            cylinders: Array.from({ length: 4 }).map((_, i) => ({
                qr_code: `OXY-DEF-E-${200 + i}`,
                serial_number: `LX-SN-${8000 + i}`,
                size: { code: 'E' }
            }))
        },
        {
            ...
            remarks: 'Scheduled Empty Cylinder Return',
            qty: 22,
            sizes: { 'J': 15, 'F': 5, 'E': 2 },
            cylinders: Array.from({ length: 22 }).map((_, i) => ({
                qr_code: `OXY-RET-M-${300 + i}`,
                serial_number: `LX-SN-${7000 + i}`,
                size: { code: i < 15 ? 'J' : i < 20 ? 'F' : 'E' }
            }))
        }
    ]

    // Mock Data for "Empty Cylinders" (for selection step)
    const MOCK_EMPTY_CYLINDERS = Array.from({ length: 12 }).map((_, i) => ({
        id: `cyl-${i}`,
        qr_code: `OXY-${1000 + i}`,
        serial_number: `SN${202400 + i}`,
        size: { code: i % 3 === 0 ? 'J' : i % 3 === 1 ? 'F' : 'E' },
        status: 'empty'
    }))

    useEffect(() => {
        // Load mock data immediately for review
        setRecentReturns(MOCK_RECENT_RETURNS)
    ...
    const loadReturnHistory = async () => {
        // Disabled real fetch for mock review
        /*
...
id,
    ...
    cylinder: pharmacy_oxygen_cylinder_inventory(
        ...
        */
        console.log("Using Mock Data for Return History")
    }
    ...
    const loadEmptyCylinders = async () => {
        ...
        setTimeout(() => {
            setEmptyCylinders(MOCK_EMPTY_CYLINDERS)
            setIsLoading(false)
        }, 500)
    }
