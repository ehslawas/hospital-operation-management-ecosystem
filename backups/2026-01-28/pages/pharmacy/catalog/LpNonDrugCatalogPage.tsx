...
import {
    ...
    updateLpNonDrug,
    deleteLpNonDrug,
    exportLpNonDrugCatalog,
    batchImportLpNonDrugs,
    type LpNonDrugCatalogFilter,
} from '@/services/pharmacy/lpNonDrugCatalogService'
...
interface FormModalProps {
    isOpen: boolean
    onClose: () => void
    onSave: (data: Partial<LpNonDrug>) => Promise<void>
    item?: LpNonDrugWithRelations | null
}

const FormModal: React.FC<FormModalProps> = ({ isOpen, onClose, onSave, item }) => {
    const [formData, setFormData] = useState<Partial<LpNonDrug>>({
        item_code: '',
        item_name: '',
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={item ? 'Edit LP Non-Drug' : 'Add New LP Non-Drug'}>
...
export default function LpNonDrugCatalogPage() {
    ...
    const [filter, setFilter] = useState<LpNonDrugCatalogFilter>({})
    const [searchTerm, setSearchTerm] = useState('')
    const [showFormModal, setShowFormModal] = useState(false)
    const [showImportModal, setShowImportModal] = useState(false)
    const [editingItem, setEditingItem] = useState<LpNonDrugWithRelations | null>(null)
    const [showDeleteModal, setShowDeleteModal] = useState(false)
    const [deletingId, setDeletingId] = useState<string | null>(null)

    const fetchKPIs = useCallback(async () => {
        if (!user?.hospital_id) return
        const { data } = await getLpNonDrugCatalogKPIs(user.hospital_id)
    ...
    const fetchItems = useCallback(async () => {
        if (!user?.hospital_id) return
        setLoading(true)
        const { data } = await getLpNonDrugCatalog(user.hospital_id, filter, page, DEFAULT_PAGE_SIZE)
        if (data) {
            setItems(data.data)
    ...
    const handleSave = async (data: Partial<LpNonDrug>) => {
        if (!user?.hospital_id || !user?.id) return

        if (editingItem) {
            const { error } = await updateLpNonDrug(editingItem.id, data)
            if (error) {
            ...
            const { error } = await createLpNonDrug(user.hospital_id, user.id, data)
            if (error) {
                toasts.error('Error', error)
            } else {
    ...
    const handleDelete = async () => {
        ...
        const { error } = await deleteLpNonDrug(deletingId)
        if (error) {
            toasts.error('Error', error)
        } else {
    ...
    const handleExport = async () => {
        ...
        const { data, error } = await exportLpNonDrugCatalog(user.hospital_id, filter)
        if (error || !data) {
        ...
        a.href = url
        a.download = `lp-non-drug-catalog-${new Date().toISOString().split('T')[0]}.csv`
        a.click()
    ...
    const handleImport = async (data: any[], _mappings: any[]) => {
        ...
        const result = await batchImportLpNonDrugs(user.hospital_id, user.id, data)
    ...
    return (
        <FinancialPageLayout
            title="LP Non-Drug Catalog"
            description="Manage Local Purchase Non-Drug items."
            icon={Package}
            breadcrumbs={[{ label: 'Catalogs', href: '#' }, { label: 'LP Non-Drugs' }]}
            actions={
