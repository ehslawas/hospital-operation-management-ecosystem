/**
 * LP Non-Drug Catalog Service
 * Handles LP non-drug catalog operations with AI-powered Excel import
 */

import { supabase } from '../supabase'
import type { ApiResponse, PaginatedResponse } from '@/types'
import type { LpNonDrug, LpNonDrugWithRelations } from '@/types/pharmacy'

export interface LpNonDrugCatalogFilter {
    search?: string
    status?: 'active' | 'inactive'
}

export interface LpNonDrugCatalogKPIs {
    total: number
    active: number
    inactive: number
}
...
export async function getLpNonDrugCatalogKPIs(hospitalId: string): Promise<ApiResponse<LpNonDrugCatalogKPIs>> {
    try {
        const { count: total } = await supabase.from('lp_non_drugs').select('*', { count: 'exact', head: true }).eq('hospital_id', hospitalId)
        const { count: active } = await supabase.from('lp_non_drugs').select('*', { count: 'exact', head: true }).eq('hospital_id', hospitalId).eq('status', 'active')
...
export async function getLpNonDrugCatalog(
    hospitalId: string,
    filter?: LpNonDrugCatalogFilter,
    page: number = 1,
    pageSize: number = 20
): Promise<ApiResponse<PaginatedResponse<LpNonDrugWithRelations>>> {
    try {
        let query = supabase.from('lp_non_drugs').select('*', { count: 'exact' }).eq('hospital_id', hospitalId)
...
export async function searchLpNonDrugs(hospitalId: string, query: string, limit: number = 10): Promise<ApiResponse<LpNonDrugWithRelations[]>> {
    try {
        if (!query || query.length < 2) return { data: [], error: null }
        const { data: items, error } = await supabase.from('lp_non_drugs').select('*').eq('hospital_id', hospitalId).or(`item_code.ilike.%${query}%,item_name.ilike.%${query}%`).limit(limit)
        if (error) throw error
...
export async function createLpNonDrug(hospitalId: string, userId: string, itemData: Partial<LpNonDrug>): Promise<ApiResponse<LpNonDrugWithRelations>> {
    try {
        const insertData: any = {
            hospital_id: hospitalId,
            item_code: itemData.item_code || `LP-ND-${Date.now()}`,
        ...
        const { data, error } = await supabase.from('lp_non_drugs').insert(insertData).select().single()
...
export async function updateLpNonDrug(itemId: string, itemData: Partial<LpNonDrug>): Promise<ApiResponse<LpNonDrugWithRelations>> {
    try {
        const { data, error } = await supabase.from('lp_non_drugs').update(itemData).eq('id', itemId).select().single()
        if (error) throw error
...
export async function deleteLpNonDrug(itemId: string): Promise<ApiResponse<void>> {
    try {
        const { error } = await supabase.from('lp_non_drugs').delete().eq('id', itemId)
        if (error) throw error
...
export async function exportLpNonDrugCatalog(hospitalId: string, filter?: LpNonDrugCatalogFilter): Promise<ApiResponse<string>> {
    try {
        const result = await getLpNonDrugCatalog(hospitalId, filter, 1, 10000)
...
export async function batchImportLpNonDrugs(
    hospitalId: string,
    userId: string,
    rawData: any[],
    ...
    try {
        console.log('[batchImportLpNonDrugs] Starting import:', rawData.length, 'rows')
        ...
        console.log('[batchImportLpNonDrugs] Data already mapped by ExcelImport, accessing fields directly')
        ...
        for (let i = 0; i < rawData.length; i += chunkSize) {
            ...
            if (upsertData.length > 0) {
                ...
                if (uniqueData.length < upsertData.length) {
                    console.log(`[batchImportLpNonDrugs] Deduped ${upsertData.length} items to ${uniqueData.length} unique items`)
                }
                ...
                if (upsertError) {
                    console.error('[batchImportLpNonDrugs] Upsert error:', upsertError)
        ...
        console.log('[batchImportLpNonDrugs] Complete. Success:', successCount, 'Errors:', errors.length)
