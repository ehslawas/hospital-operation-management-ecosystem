...
interface ReviewModalProps {
  ...
  status: 'PENDING_REVIEW' | 'UNDER_REVIEW' | 'PENDING_APPROVAL' | 'APPROVED' | 'ISSUED' | 'REJECTED' | 'CANCELLED';
  onUpdateRequest: (requestId: string, updates: Partial<DepartmentRequest>) => void;
}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-[95vh] overflow-hidden border border-slate-200">
        ...
        <div className="flex h-[calc(95vh-120px)]">
          ...
          <div className="w-96 border-r border-slate-200 bg-slate-50/30 overflow-y-auto">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Request Queue</h3>
              ...
              <div className="p-8">
                ...
                <div className="mb-8">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <h3 className="text-3xl font-bold text-slate-900 mb-2">{selectedRequest.requestNumber}</h3>
                      <div className="flex items-center gap-6 text-slate-600">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">{new Date(selectedRequest.requestedAt).toLocaleDateString()}</span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {currentItems.map((item) => {
                      ...
                      const mockBatch = `B${Math.floor(Math.random() * 9000) + 1000}`;
                      const mockExpiry = new Date();
                      mockExpiry.setFullYear(mockExpiry.getFullYear() + Math.floor(Math.random() * 3) + 1);
                      mockExpiry.setMonth(mockExpiry.getMonth() + Math.floor(Math.random() * 12));
                      const mockAvgUsage = Math.floor(Math.random() * 500) + 100;
                      const packSizes = [10, 20, 30, 50, 100, 250, 500, 1000];
                      const packSize = packSizes[Math.floor(Math.random() * packSizes.length)];
                        ...
                        <div key={item.id} className="bg-white border border-slate-200 rounded-lg p-4 hover:shadow-sm transition-shadow">
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-3 flex-1">
                              <div className={`w-2 h-2 rounded-full ${
                                item.drugCode && item.drugCode !== 'N/A' && 
                                (item.itemName.toLowerCase().includes('mg') || 
                                 item.itemName.toLowerCase().includes('tablet') ||
                                 item.itemName.toLowerCase().includes('capsule') ||
                                 item.itemName.toLowerCase().includes('syrup') ||
                          ...
                          <div className="grid grid-cols-6 gap-4 text-xs">
                            <div>
                              ...
                              <p className="font-medium text-slate-700">pack of {packSize} {getPackUnit(item.unit)}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 mb-1">Batch/Exp</p>
                              <p className="font-medium text-slate-700">{mockBatch}</p>
                              <p className="text-slate-500">{mockExpiry.toLocaleDateString('en-MY', { month: 'short', year: '2-digit' })}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 mb-1">Balance</p>
                              <p className="font-bold text-green-600">{mockBalance.toLocaleString()}</p>
                            ...
                            <div>
                              ...
                              <p className="font-medium text-slate-700">{mockAvgUsage.toLocaleString()}</p>
