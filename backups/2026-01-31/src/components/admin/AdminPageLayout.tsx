...
export const AdminPageLayout: React.FC<AdminPageLayoutProps> = ({
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/20 relative">
            {/* Ambient Background Shapes */}
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-indigo-100/20 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-blue-100/20 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2 pointer-events-none" />
            ...
            <div className="p-6 space-y-6 max-w-[1600px] mx-auto relative z-10">
                ...
                <nav className="flex items-center text-xs text-slate-500 mb-2 font-medium tracking-wide uppercase">
                    <span className="hover:text-indigo-900 cursor-pointer transition-colors">Admin</span>
                    {breadcrumbs.map((crumb, index) => (
                        <React.Fragment key={index}>
                            <ChevronRight className="w-3 h-3 mx-2 text-slate-300" />
                            <span
                                className={crumb.href ? "hover:text-indigo-900 cursor-pointer transition-colors" : "text-indigo-900 font-bold"}
                            >
                                {crumb.label}
                            </span>
                        </React.Fragment>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    >
                        {Icon && (
                            <div className="p-3.5 glass rounded-2xl shadow-lg shadow-indigo-900/5 mt-1">
              ...
              ${notice.type === 'warning'
                                ...
                                : notice.type === 'success'
                                    ...
                                    : 'bg-indigo-50/50 border-indigo-200 border-l-indigo-500'
                            }
            `}>
