...
export async function generateCCPenaltyPdf(penalty: any): Promise<jsPDF> {
  ...
  const amountVal = penalty.penalty_amount || 0
  ...
  const totalAmount = Number(penalty.penalty_amount || 0)
  tableRows.push([
    {
  ...
  doc.setFont('times', 'bold')
  const contactName = penalty.verified_by_name || penalty.checked_by_name || (penalty.verified_by?.full_name) || 'Kamriah Binti Haji Mail'
  const contactPhone = penalty.verified_by_phone || '085-283781 (Samb. 206)'
  ...
  const dendaVal = Number(penalty.penalty_amount || 0)
