...
import type { Hospital, HospitalWithAdmin } from '@/types'
...
export interface FacilityStatistics {
  ...
  moduleBreakdown: { code: string; name: string; enabled: boolean }[]
  roleBreakdown: { roleName: string; userCount: number }[]
}
...
export async function getFacilityStatistics(facilityId: string): Promise<FacilityStatistics> {
  if (isSupabaseConfigured()) {
    ...
    // Supabase is required for Facility Statistics
    throw new Error('Supabase is not configured. Facility statistics require database connection.')
  }
}
...
export async function getFacilityLogs(facilityId: string, limit: number = 10) {
  if (isSupabaseConfigured()) {
    ...
    throw new Error('Supabase is not configured. Facility logs require database connection.')
  }
}
