...
import type { AdminPembangunan, AdminPembangunanSummary } from '@/types/adminOperations.types';
import {
...
// ============================================================================
// ADMIN PEMBANGUNAN PAGE - Hospital Administrator Exclusive
// Theme: Emerald/Teal to differentiate from Warrant (Purple)
// ============================================================================

export const AdminPembangunanPage: React.FC = () => {
    ...
    // ========== STATE MANAGEMENT ==========
    const [warrants, setWarrants] = useState<AdminPembangunan[]>([]);
    const [summary, setSummary] = useState<AdminPembangunanSummary | null>(null);
    const [isLoading, setIsLoading] = useState(false);
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());
    const [filterProgram, setFilterProgram] = useState<string>('all');
    const [expandedPrograms, setExpandedPrograms] = useState<Set<string>>(new Set(['P42']));
    ...
    const fetchData = async () => {
        ...
        try {
            const [warrantsData, summaryData] = await Promise.all([
                adminPembangunanService.getAdminPembangunan(hospitalId, { fiscalYear: selectedYear }),
                adminPembangunanService.getAdminPembangunanSummary(hospitalId, selectedYear)
            ]);
            setWarrants(warrantsData);
            setSummary(summaryData);
        } catch (err) {
            console.error('Failed to fetch admin pembangunan:', err);
    ...
    // ========== FORM HANDLERS ==========
    const handleEdit = (warrant: AdminPembangunan) => {
        navigate(ROUTES.ADMIN_OPERATIONS_PEMBANGUNAN_EDIT.replace(':id', warrant.id));
    };

    const handleDelete = async (warrant: AdminPembangunan) => {
        if (!window.confirm('Are you sure you want to delete this record?')) return;

        try {
            await adminPembangunanService.deleteAdminPembangunan(warrant.id);
        ...
        <div className="space-y-6 p-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">
                        Admin Pembangunan P42
                    </h1>
                    <p className="text-slate-500 mt-1">
                        Manage development budget allocations (Sewaan & PSH)
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        onClick={() => navigate(ROUTES.ADMIN_OPERATIONS_PEMBANGUNAN_CREATE)}
                        className={`bg-gradient-to-r ${ADMIN_PEMBANGUNAN_GRADIENT.primary} text-white`}
                    >
            ...
            <div className="flex items-center gap-2">
                ...
                <Select
                    ...
                    className="w-32 border-emerald-200 focus:ring-emerald-500"
                >
                        ...
                        <CardContent className="pt-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    <p className="text-sm text-emerald-600 font-medium truncate">Total Allocation</p>
                    ...
                    <Card className="border-teal-200 bg-gradient-to-br from-teal-50 to-teal-100">
                        <CardContent className="pt-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    <p className="text-sm text-teal-600 font-medium truncate">Total Expenses</p>
                    ...
                    <Card className="border-cyan-200 bg-gradient-to-br from-cyan-50 to-cyan-100">
                        <CardContent className="pt-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    <p className="text-sm text-cyan-600 font-medium truncate">Balance</p>
                                    <p className="text-2xl font-bold text-cyan-900 truncate">
                    ...
                    <Card className="border-slate-200 bg-gradient-to-br from-slate-50 to-slate-100">
                        <CardContent className="pt-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    <p className="text-sm text-slate-600 font-medium truncate">Records</p>
            ...
            {!isLoading && summary && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {ADMIN_PEMBANGUNAN_PROGRAMS.map(prog => {
                        ...
                        return (
                            <Card key={prog.code} className="overflow-hidden border-teal-200">
            ...
            {/* Filter Bar */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                <div className="md:col-span-2 relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <Input
                        placeholder="Search document no, description..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="pl-10 border-slate-200 focus:ring-emerald-500 h-11"
                    />
                </div>
                <div className="flex gap-2 items-center bg-slate-50 border border-slate-200 rounded-xl px-3 h-11">
                    <Building2 className="w-4 h-4 text-emerald-600" />
                    <Select
                        value={filterProgram}
                        onValueChange={setFilterProgram}
                        className="flex-1 border-none focus:ring-0 h-9 bg-transparent"
                    >
                        <SelectItem value="all">All Programs</SelectItem>
                        {ADMIN_PEMBANGUNAN_PROGRAMS.map(prog => (
                            <SelectItem key={prog.code} value={prog.code}>{prog.label} ({prog.code})</SelectItem>
                        ))}
                    </Select>
                </div>
            </div>
            ...
            <Card className="border-slate-200">
                <CardHeader>
                    <CardTitle className="text-lg text-slate-800">Allocations</CardTitle>
                </CardHeader>
                <CardContent>
                    {isLoading ? (
                        <div className="flex items-center justify-center h-32">
                        ...
                        <div className="text-center py-12 text-slate-500">
                            ...
                            <p className="text-sm mt-1">Create a new allocation to get started</p>
                        </div>
                    ) : (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {filteredWarrants.map((warrant) => (
                                        <TableRow key={warrant.id} className="hover:bg-emerald-50/50">
                                            <TableCell className="font-mono text-sm">
                                                {formatDate(warrant.pembangunan_date)}
                                            </TableCell>
                                            ...
                                            <TableCell className="max-w-[150px] truncate" title={warrant.objek_code}>
...
export default AdminPembangunanPage;
