...
import { WARRANT_VOTE_ACTIVITIES, WARRANT_DEPARTMENTS } from '@/services/pharmacy/warrantService'
...
export const APPLAllocationPage: React.FC = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      showSuccess('Export successful', 'APPL allocation report generated')
    } catch (error) {
      console.error('Error exporting PDF:', error)
      showError('Export failed', 'Failed to generate APPL allocation report')
    } finally {
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      a.href = url
      a.download = `appl-allocation-report-FY${selectedYear}.csv`
  ...
  return (
    <FinancialPageLayout
      title="APPL Allocation"
      description="Track expenses from Purchase Orders (PO) and Local Purchase Orders (LPO) linked to warrants (vote code 990102)"
      icon={Wallet}
      breadcrumbs={[{ label: 'APPL Allocation' }]}
