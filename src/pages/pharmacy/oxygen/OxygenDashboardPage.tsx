            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <p className="text-slate-500 font-semibold text-xs">
                  Real-time oxygen cylinder tracking • All data live from Supabase
                </p>
              </div>
            </div>
            <button
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              <QrCode className="w-5 h-5 text-[#00a68a]" />
              Cylinder QR Label Generator
            </h2>
            <p className="text-xs text-gray-500 mt-0.5">
              Select any cylinder from the database to generate a printable scan label.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
              <h3 className="text-sm font-bold text-gray-800 border-b border-gray-100 pb-3">Select Cylinder</h3>
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-wide">Cylinder Serial</label>
                  <select 
                    value={selectedCylinderId}
                    onChange={(e) => setSelectedCylinderId(e.target.value)}
                    className="w-full mt-1.5 border border-gray-200 rounded-xl p-3 text-xs outline-none bg-white text-gray-700 focus:ring-2 focus:ring-[#00a68a]/20 focus:border-[#00a68a] transition-all font-mono"
                  >
                    <option value="">-- Choose Cylinder --</option>
                    {cylinders.map(c => (
                      <option key={c.id} value={c.id}>{c.serial_number} ({c.type_info?.type_name || 'Standard'})</option>
                    ))}
                  </select>
                </div>
                <button 
                  onClick={() => setGeneratedLabel(selectedCylinder || null)}
                  disabled={!selectedCylinderId}
                  className="w-full py-3.5 bg-[#00a68a] hover:bg-[#008f76] disabled:opacity-50 text-white rounded-xl text-xs font-bold shadow-sm transition-all flex items-center justify-center gap-1.5"
                >
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center min-h-[300px] text-center">
              {generatedLabel ? (
                <div className="space-y-4 flex flex-col items-center w-full">
                  <div className="p-6 border-2 border-[#00a68a]/30 bg-slate-50 rounded-2xl flex flex-col items-center max-w-[280px] w-full shadow-inner">
                    {/* Simulated SVG QR */}
                    <svg className="w-36 h-36 border border-gray-200 rounded-lg p-1 bg-white" viewBox="0 0 100 100" fill="none">
                    ...
                    <div className="mt-3 text-center">
                      <p className="text-xs font-mono font-bold text-gray-800 tracking-wider">{generatedLabel.serial_number}</p>
                      <p className="text-[9px] text-gray-500 font-semibold mt-0.5">{generatedLabel.type_info?.type_name}</p>
                      <span className="text-[7px] font-black uppercase text-[#00a68a] tracking-widest mt-1 block">KKM MEDICAL OXYGEN</span>
                    </div>
                  </div>
                  <button className="px-4 py-2 border border-gray-200 hover:bg-slate-50 text-gray-700 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-1.5">
                ...
                <div className="space-y-2 text-gray-400">
                  ...
                  <p className="text-xs font-medium px-4">Please select an active cylinder to preview its QR tracking code.</p>
                </div>
              )}
            </div>
          </div>
