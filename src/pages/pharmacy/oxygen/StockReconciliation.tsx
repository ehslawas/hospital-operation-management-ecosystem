...
const StockReconciliation: React.FC = () => {
    ...
    const processTimeoutRef = React.useRef<NodeJS.Timeout | null>(null)
    ...
    const processScanQueue = async () => {
        if (scanQueue.length === 0 || isProcessingQueue) return

        setIsProcessingQueue(true)
        const codesToProcess = [...scanQueue]
        setScanQueue([])

        let successCount = 0
        let duplicateCount = 0
        let errorCount = 0
        const newCylinders: OxygenCylinderInventoryWithRelations[] = []

        // Process sequentially to potential race conditions on DB
        // In a real bulk API we would send all codes at once, but for now we'll batch the UI updates
        for (const code of codesToProcess) {
            const res = await findCylinderByQR(user!.hospital_id, code)

            if (!res.error && res.data) {
                // Check against both currently staged AND newly found in this batch
                const isDuplicateStaged = stagedCylinders.some(c => c.id === res.data!.id)
                const isDuplicateBatch = newCylinders.some(c => c.id === res.data!.id)

                if (isDuplicateStaged || isDuplicateBatch) {
                    duplicateCount++
                } else {
                    newCylinders.push(res.data)
                    successCount++
                }
            } else {
                errorCount++
        ...
        if (newCylinders.length > 0) {
            setStagedCylinders(prev => [...prev, ...newCylinders])
        }

        // Single summary toast
        if (successCount > 0) {
            toast.success('Batch Complete', `${successCount} cylinder(s) added${duplicateCount ? `, ${duplicateCount} duplicate(s) skipped` : ''}`)
        }
        if (errorCount > 0) {
            toast.warning('Scan Issues', `${errorCount} code(s) not found`)
        } else if (successCount === 0 && duplicateCount > 0) {
            toast.info('No New Items', `Skipped ${duplicateCount} duplicate cylinder(s)`)
        }
    ...
    useEffect(() => {
        if (scanQueue.length > 0 && !isProcessingQueue) {
            if (processTimeoutRef.current) clearTimeout(processTimeoutRef.current)
            processTimeoutRef.current = setTimeout(processScanQueue, 500)
        }
        return () => {
            if (processTimeoutRef.current) clearTimeout(processTimeoutRef.current)
        }
    }, [scanQueue, isProcessingQueue])
    ...
    const handleSearch = async (codeToSearch?: string) => {
        ...
        if (isBulkMode) {
            // 1. Immediate Client-Side Duplicate Check
            // Check if already in pending queue
            if (scanQueue.includes(query)) {
                return
            }

            // Check if already staged (avoid DB call entirely)
            // match against QR code, Serial Number, or ID
            const isAlreadyStaged = stagedCylinders.some(c =>
                c.qr_code?.toLowerCase() === query.toLowerCase() ||
                c.serial_number?.toLowerCase() === query.toLowerCase() ||
                c.id === query
            )

            if (isAlreadyStaged) {
                // Visual feedback for duplicate without blocking/spamming
                toast.info('Duplicate', 'Item already in list')
                setSearchQuery('')
                return
            }

            // 2. Queue valid new scan
            setScanQueue(prev => [...prev, query])
            setSearchQuery('')
            return
        }
    ...
    return (
        <div className="max-w-[1400px] mx-auto p-4 md:p-6 space-y-6 md:space-y-8 animate-in fade-in duration-500">
            ...
            {isBulkMode && scanQueue.length > 0 && (
                <div className="fixed bottom-20 left-1/2 -translate-x-1/2 bg-indigo-600 text-white px-6 py-3 rounded-full shadow-lg shadow-indigo-200 z-50 flex items-center gap-3 animate-pulse">
                    <RefreshCw className="h-5 w-5 animate-spin" />
                    <span className="font-bold">Processing {scanQueue.length} scan(s)...</span>
                </div>
            )}
