/**
 * Clinic Facility Catalog Page
 * Modern, professional system to display and manage list of clinics in Malaysia
...
import {
  ...
  deleteClinicFacility,
  exportClinicFacilities,
  importClinicsFromMOH,
} from '@/services/pharmacy/clinicFacilityCatalogService'
import type {
...
interface EditModalProps {
  ...
  facility?: ClinicFacilityWithRelations | null
}
...
const EditModal: React.FC<EditModalProps> = ({ isOpen, onClose, onSave, facility }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={facility ? 'Edit Clinic Facility' : 'Add Clinic Facility'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <Input
            ...
            required
            placeholder="e.g., Klinik Kesihatan Kuala Lumpur"
          />
        </div>
...
// =====================================================
// MAIN CLINIC FACILITY CATALOG PAGE
// =====================================================
...
export const ClinicFacilityCatalogPage: React.FC = () => {
  ...
  // State
  const [facilities, setFacilities] = useState<ClinicFacilityWithRelations[]>([])
  const [kpis, setKpis] = useState<ClinicFacilityCatalogKPIs>({
    total: 0,
    by_state: [],
    by_city: [],
  })
  ...
  const [selectedFacility, setSelectedFacility] = useState<ClinicFacilityWithRelations | null>(null)
  const [showDeleteModal, setShowDeleteModal] = useState(false)
  const [facilityToDelete, setFacilityToDelete] = useState<ClinicFacilityWithRelations | null>(null)
  ...
  const loadFacilities = async () => {
    ...
    setIsLoading(true)
    try {
      const filter: ClinicFacilityCatalogFilter = {
      ...
      const result = await getClinicFacilities(user.hospital_id, filter)
      if (result.data) {
      ...
      showError('Error', 'Failed to load clinic facilities')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const loadKPIs = async () => {
    ...
    try {
      const result = await getClinicFacilityKPIs(user.hospital_id)
      if (result.data) {
        setKpis(result.data)
      }
    } catch (error) {
  ...
  const handleSave = async (formData: any) => {
    ...
    try {
      if (selectedFacility) {
        // Update
        const result = await updateClinicFacility(selectedFacility.id, formData)
        if (result.data) {
          showSuccess('Success', 'Clinic facility updated successfully')
          loadFacilities()
          loadKPIs()
        } else if (result.error) {
          showError('Error', result.error)
        ...
        // Create
        const result = await createClinicFacility(user.hospital_id, formData)
        if (result.data) {
          showSuccess('Success', 'Clinic facility created successfully')
          loadFacilities()
          loadKPIs()
        } else if (result.error) {
          showError('Error', result.error)
      ...
      console.error('Error saving facility:', error)
      showError('Error', 'Failed to save clinic facility')
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteClinicFacility(facilityToDelete.id)
      if (result.error) {
        showError('Error', result.error)
      } else {
        showSuccess('Success', 'Clinic facility deleted successfully')
      ...
      showError('Error', 'Failed to delete clinic facility')
    }
  }
  ...
  const handleFetchFromMOH = async () => {
    ...
    if (!confirm('This will fetch and import clinics from the MOH website. This may take a few minutes. Continue?')) {
      return
    }
    ...
    try {
      const result = await importClinicsFromMOH(
        user.hospital_id,
        (progress) => {
      ...
      if (result.data) {
        ...
        if (success > 0) {
          showSuccess(
            'Success',
            `Successfully imported ${success} clinic${success !== 1 ? 's' : ''} from MOH website${failed > 0 ? ` (${failed} failed)` : ''}`
          )
          // Reload facilities
          ...
          showError(
            'Import Failed',
            `Failed to import ${failed} clinic${failed !== 1 ? 's' : ''}. ${errors.slice(0, 3).join('; ')}${errors.length > 3 ? '...' : ''}`
          )
        }
      } else if (result.error) {
        showError('Error', result.error)
      ...
      showError('Error', 'Failed to fetch clinics from MOH website')
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const result = await exportClinicFacilities(user.hospital_id, filter)
      if (result.data) {
        // Download file
        const url = window.URL.createObjectURL(result.data)
        const link = document.createElement('a')
        link.href = url
        link.download = `clinic_facilities_${new Date().toISOString().split('T')[0]}.csv`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        window.URL.revokeObjectURL(url)

        showSuccess('Success', 'Clinic facilities exported successfully')
      } else if (result.error) {
        showError('Error', result.error)
      }
    } catch (error) {
      console.error('Error exporting facilities:', error)
      showError('Error', 'Failed to export clinic facilities')
    } finally {
  ...
  return (
    <FinancialPageLayout
      title="Clinic Facility Catalog"
      description="Manage list of clinics in Malaysia. Data sourced from Ministry of Health Malaysia (MOH)."
      icon={Building2}
      breadcrumbs={[{ label: 'Catalogs', href: '#' }, { label: 'Clinics' }]}
      actions={
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 text-white shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm font-medium text-blue-50">Total Clinics</span>
              </div>
        ...
        <div className="glass-card rounded-xl p-4 flex flex-col gap-4 border border-white/40 shadow-sm">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search clinics..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 h-10 bg-slate-50 border-transparent rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-100 outline-none text-sm transition-all" />
            </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={5} className="text-center py-10 text-slate-500">No clinics found</TableCell></TableRow>
              ) : (
...
export default ClinicFacilityCatalogPage
