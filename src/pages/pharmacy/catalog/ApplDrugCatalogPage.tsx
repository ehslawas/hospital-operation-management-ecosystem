...
import { Search, Plus, Download, Edit, Trash2, FileUp, Package, CheckCircle2, XCircle, Pill } from 'lucide-react'
...
import {
    getApplDrugCatalogKPIs,
    getApplDrugCatalog,
    createApplDrug,
    updateApplDrug,
    deleteApplDrug,
    exportApplDrugCatalog,
    batchImportApplDrugs,
    type ApplDrugCatalogFilter,
} from '@/services/pharmacy/applDrugCatalogService'
...
interface FormModalProps {
    isOpen: boolean
    onClose: () => void
    onSave: (data: Partial<ApplDrug>) => Promise<void>
    item?: ApplDrugWithRelations | null
}

const FormModal: React.FC<FormModalProps> = ({ isOpen, onClose, onSave, item }) => {
    const [formData, setFormData] = useState<Partial<ApplDrug>>({
        item_code: '',
        item_name: '',
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={item ? 'Edit APPL Drug' : 'Add New APPL Drug'}>
...
// Main Component
export default function ApplDrugCatalogPage() {
    const toasts = useToast()
    const { user } = useAuthStore()

    const [kpis, setKpis] = useState({ total: 0, active: 0, inactive: 0 })
    const [items, setItems] = useState<ApplDrugWithRelations[]>([])
    const [loading, setLoading] = useState(true)
    const [page, setPage] = useState(1)
    const [totalPages, setTotalPages] = useState(1)
    const [filter, setFilter] = useState<ApplDrugCatalogFilter>({})
    const [searchTerm, setSearchTerm] = useState('')
    const [showFormModal, setShowFormModal] = useState(false)
    const [showImportModal, setShowImportModal] = useState(false)
    const [editingItem, setEditingItem] = useState<ApplDrugWithRelations | null>(null)
    const [showDeleteModal, setShowDeleteModal] = useState(false)
    const [deletingId, setDeletingId] = useState<string | null>(null)

    const fetchKPIs = useCallback(async () => {
        if (!user?.hospital_id) return
        const { data } = await getApplDrugCatalogKPIs(user.hospital_id)
    ...
    const fetchItems = useCallback(async () => {
        if (!user?.hospital_id) return
        setLoading(true)
        const { data } = await getApplDrugCatalog(user.hospital_id, filter, page, DEFAULT_PAGE_SIZE)
        if (data) {
            setItems(data.data)
    ...
    const handleSave = async (data: Partial<ApplDrug>) => {
        if (!user?.hospital_id || !user?.id) return

        if (editingItem) {
            const { error } = await updateApplDrug(editingItem.id, data)
            if (error) {
            ...
            const { error } = await createApplDrug(user.hospital_id, user.id, data)
            if (error) {
                toasts.error('Error', error)
            } else {
    ...
    const handleDelete = async () => {
        ...
        const { error } = await deleteApplDrug(deletingId)
        if (error) {
            toasts.error('Error', error)
        } else {
    ...
    const handleExport = async () => {
        ...
        const { data, error } = await exportApplDrugCatalog(user.hospital_id, filter)
        if (error || !data) {
        ...
        a.href = url
        a.download = `appl-drug-catalog-${new Date().toISOString().split('T')[0]}.csv`
        a.click()
    ...
    const handleImport = async (data: any[], _mappings: any[]) => {
        ...
        const result = await batchImportApplDrugs(user.hospital_id, user.id, data)
    ...
    return (
        <FinancialPageLayout
            title="APPL Drug Catalog"
            description="Manage Approved Product List (APPL) Drug items."
            icon={Pill}
            breadcrumbs={[{ label: 'Catalogs', href: '#' }, { label: 'APPL Drugs' }]}
            actions={
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 text-white shadow-lg relative overflow-hidden">
                        <div className="relative z-10">
                            <div className="flex items-center gap-3 mb-2">
                                <div className="p-2 bg-white/20 rounded-lg"><Pill className="w-5 h-5 text-blue-50" /></div>
