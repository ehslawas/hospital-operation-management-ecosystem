...
export default function FacilityCatalogPage() {
  ...
  const handleAddWithSections = (e: React.FormEvent) => {
    e.preventDefault()
    ...
    addToast({
      ...
      message: `"${newItem.name}" has been successfully added to your facility catalog with verified specs.`,
      type: 'success',
    })
  }
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const drawHeader = () => {
        ...
        doc.text(`Kategori Carian: ${activeTab === 'all' ? 'Semua Ubat & Bukan Ubat' : activeTab === 'Drug' ? 'Ubat Sahaja' : 'Bukan Ubat Sahaja'}`, margin, 48)
        doc.text(`Tarikh Laporan: ${new Date().toLocaleDateString('en-MY')}`, pageWidth - margin, 48, { align: 'right' })
        doc.text(`Jumlah Item Berdaftar: ${filteredItems.length} rekod`, margin, 52)
        doc.text(`Dijana Oleh: ${activeUserName}`, pageWidth - margin, 52, { align: 'right' })
      }
      ...
      autoTable(doc, {
        startY: 56,
        head: [['Bil', 'Drug/Non Drug Name', 'Item Code', 'Packaging Description', 'Procurement', 'Category', 'Harga/PKU']],
        ...
        didDrawPage: (data) => {
          if (data.pageNumber > 1) {
            ...
            doc.text('REKOD KATALOG KEMUDAHAN FARMASI - HOSPITAL DAERAH LAWAS', margin, 12)
          ...
          doc.text(`Halaman ${data.pageNumber}`, pageWidth - margin, pageHeight - 7, { align: 'right' })
          doc.text('Laporan Katalog Kemudahan Farmasi Hospital Daerah Lawas - Dokumen Cetakan Rasmi', margin, pageHeight - 7)
        }
      })
      ...
      doc.text('SEKSYEN SYARAT KELULUSAN & PENGESAHAN DOKUMEN', margin, finalY)
      
      doc.setFont('helvetica', 'normal')
      doc.setFontSize(7.5)
      doc.setTextColor(71, 85, 105)
      doc.text('Dokumen cetakan katalog ini merupakan salinan daftar inventori rasmi farmasi Hospital Daerah Lawas bagi tujuan pengesahan kualiti, piawaian harga, dan rekod rujukan rasmi perolehan.', margin, finalY + 4, { maxWidth: contentWidth })

      const sigY = finalY + 14
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(8)
      doc.setTextColor(15, 23, 42)
      doc.text('Disediakan Oleh,', margin + 15, sigY)
      doc.text('Disahkan Oleh,', pageWidth / 2 - 20, sigY)
      doc.text('Diluluskan Oleh,', pageWidth - margin - 50, sigY)

      const lineY = sigY + 14
      doc.setLineWidth(0.25)
      doc.setDrawColor(148, 163, 184)
      doc.line(margin + 10, lineY, margin + 65, lineY)
      doc.line(pageWidth / 2 - 30, lineY, pageWidth / 2 + 25, lineY)
      doc.line(pageWidth - margin - 55, lineY, pageWidth - margin - 5, lineY)

      doc.setFont('helvetica', 'normal')
      doc.setFontSize(7.5)
      doc.setTextColor(100, 116, 139)
      doc.text('(Cop & Tandatangan)', margin + 20, lineY + 3.5)
      doc.text('(Cop Pengesahan)', pageWidth / 2 - 17, lineY + 3.5)
      doc.text('(Ketua Pegawai Farmasi)', pageWidth - margin - 47, lineY + 3.5)

      doc.text('Tarikh: ______________', margin + 20, lineY + 7.5)
      doc.text('Tarikh: ______________', pageWidth / 2 - 17, lineY + 7.5)
      doc.text('Tarikh: ______________', pageWidth - margin - 47, lineY + 7.5)

      const pdfBlob = doc.output('blob')
      const pdfUrl = URL.createObjectURL(pdfBlob)
      const link = document.createElement('a')
      link.href = pdfUrl
      link.download = `Katalog_Kemudahan_Farmasi_Lawas_${new Date().toISOString().split('T')[0]}.pdf`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      setTimeout(() => URL.revokeObjectURL(pdfUrl), 1000)
      addToast({
        title: 'Katalog Berjaya Dimuat Turun',
        message: 'Laporan daftar katalog PDF sedia untuk dicetak dan disahkan.',
        type: 'success'
      })
    } catch (error: any) {
      console.error('PDF Export Error:', error)
      addToast({
        title: 'Gagal Eksport PDF',
        message: error.message || 'Ralat berlaku semasa menyediakan fail PDF.',
        type: 'warning'
      })
    } finally {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 py-2">
                  ...
                  <div className="lg:col-span-4 border-l border-slate-100 pl-6 space-y-4">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                      ...
                      <Badge variant="ghost" className="text-[10px] font-bold bg-slate-100 text-slate-500">
                        Live
                      </Badge>
                    </div>
      ...
      <AnimatePresence>
        {isAddModalOpen && (
          <Modal
            ...
            size={selectedMasterItem || showCreateCustomForm ? "7xl" : "5xl"}
          >
              ...
              {(selectedMasterItem || showCreateCustomForm) && (
                <form onSubmit={handleAddWithSections} className="space-y-6">
                  ...
                  <p className="text-[11px] font-black text-rose-500 bg-rose-50/50 border border-rose-100/50 px-3 py-2 rounded-xl uppercase tracking-wider">
                    ⚠️ CRITICAL VERIFICATION: Please review and set all 4 specification sections before saving this item to the active hospital facility registry.
                  </p>
