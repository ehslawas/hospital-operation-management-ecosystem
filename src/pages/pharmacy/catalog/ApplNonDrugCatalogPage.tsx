...
import { Search, Plus, Download, Edit, Trash2, FileUp, Package, CheckCircle2, XCircle, Box } from 'lucide-react'
...
import {
    getApplNonDrugCatalogKPIs,
    getApplNonDrugCatalog,
    createApplNonDrug,
    updateApplNonDrug,
    deleteApplNonDrug,
    exportApplNonDrugCatalog,
    batchImportApplNonDrugs,
    type ApplNonDrugCatalogFilter,
} from '@/services/pharmacy/applNonDrugCatalogService'
...
interface FormModalProps {
    isOpen: boolean
    onClose: () => void
    onSave: (data: Partial<ApplNonDrug>) => Promise<void>
    item?: ApplNonDrugWithRelations | null
}

const FormModal: React.FC<FormModalProps> = ({ isOpen, onClose, onSave, item }) => {
    const [formData, setFormData] = useState<Partial<ApplNonDrug>>({
        item_code: '',
        item_name: '',
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={item ? 'Edit APPL Non-Drug' : 'Add New APPL Non-Drug'}>
...
// Main Component
export default function ApplNonDrugCatalogPage() {
    const toasts = useToast()
    const { user } = useAuthStore()
    const isSessionReady = useIsSessionReady()

    const [kpis, setKpis] = useState({ total: 0, active: 0, inactive: 0 })
    const [items, setItems] = useState<ApplNonDrugWithRelations[]>([])
    const [loading, setLoading] = useState(true)
    const [page, setPage] = useState(1)
    const [totalPages, setTotalPages] = useState(1)
    const [filter, setFilter] = useState<ApplNonDrugCatalogFilter>({})
    const [searchTerm, setSearchTerm] = useState('')
    const [showFormModal, setShowFormModal] = useState(false)
    const [showImportModal, setShowImportModal] = useState(false)
    const [editingItem, setEditingItem] = useState<ApplNonDrugWithRelations | null>(null)
    const [showDeleteModal, setShowDeleteModal] = useState(false)
    const [deletingId, setDeletingId] = useState<string | null>(null)

    const fetchKPIs = useCallback(async () => {
        if (!user?.hospital_id) return
        const { data } = await getApplNonDrugCatalogKPIs(user.hospital_id)
    ...
    const fetchItems = useCallback(async () => {
        if (!user?.hospital_id) return
        setLoading(true)
        const { data } = await getApplNonDrugCatalog(user.hospital_id, filter, page, DEFAULT_PAGE_SIZE)
        if (data) {
            setItems(data.data)
    ...
    const handleSave = async (data: Partial<ApplNonDrug>) => {
        if (!user?.hospital_id || !user?.id) return

        if (editingItem) {
            const { error } = await updateApplNonDrug(editingItem.id, data)
            if (error) {
            ...
            const { error } = await createApplNonDrug(user.hospital_id, user.id, data)
            if (error) {
                toasts.error('Error', error)
            } else {
    ...
    const handleDelete = async () => {
        ...
        const { error } = await deleteApplNonDrug(deletingId)
        if (error) {
            toasts.error('Error', error)
        } else {
    ...
    const handleExport = async () => {
        ...
        const { data, error } = await exportApplNonDrugCatalog(user.hospital_id, filter)
        if (error || !data) {
        ...
        a.href = url
        a.download = `appl-non-drug-catalog-${new Date().toISOString().split('T')[0]}.csv`
        a.click()
    ...
    const handleImport = async (data: any[], _mappings: any[]) => {
        ...
        const result = await batchImportApplNonDrugs(user.hospital_id, user.id, data)
    ...
    return (
        <FinancialPageLayout
            title="APPL Non-Drug Catalog"
            description="Manage Approved Product List (APPL) Non-Drug items."
            icon={Box}
            breadcrumbs={[{ label: 'Catalogs', href: '#' }, { label: 'APPL Non-Drugs' }]}
            actions={
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 text-white shadow-lg relative overflow-hidden">
                        <div className="relative z-10">
                            <div className="flex items-center gap-3 mb-2">
                                <div className="p-2 bg-white/20 rounded-lg"><Box className="w-5 h-5 text-blue-50" /></div>
