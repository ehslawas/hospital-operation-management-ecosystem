export const ROUTES = {
  LOGIN: '/login',
  DASHBOARD: '/dashboard',
  HUB: '/hub',
  HUB_CYLINDER: '/hub/cylinder',
  HUB_INVENTORY: '/hub/inventory',
  HUB_WARRANT: '/hub/warrant',
  HUB_SURAT: '/hub/surat',
  HUB_BORANG: '/hub/borang',
  HUB_SUHU: '/suhu',
  HUB_SUHU_DASHBOARD: '/suhu/dashboard',
  HUB_SUHU_DEPARTMENT: '/suhu/department/:departmentId',
  HUB_SUHU_UNIT: '/suhu/unit/:unitId',
  HUB_SUHU_BREACHES: '/suhu/breaches',
  HUB_SUHU_ADMIN: '/suhu/admin',
  HUB_ADMIN: '/hub/admin',
  HUB_PEROLEHAN: '/perolehan',
  PEROLEHAN: '/perolehan',
  PEROLEHAN_DASHBOARD: '/perolehan/dashboard',
  PEROLEHAN_PENGURUSAN: '/perolehan/pengurusan',
  PEROLEHAN_PEMBANGUNAN: '/perolehan/pembangunan',
  PEROLEHAN_ORDERS: '/perolehan/orders',
  PEROLEHAN_PAYMENTS: '/perolehan/payments',
  PEROLEHAN_CATALOG: '/perolehan/catalog',
  HUB_GALLERY: '/hub/gallery',
  HUB_MEMO: '/hub/memo',
  HUB_CROSSBORDER: '/hub/crossborder',
  HUB_FILE: '/hub/file',
  HUB_FORMULARI: '/hub/formulari',
  FORMULARI_DASHBOARD: '/formulari/dashboard',
  FORMULARI_DRUG_DETAIL: '/formulari/drug/:id',
  FORMULARI_HAM: '/formulari/ham',
  FORMULARI_LASA: '/formulari/lasa',
  FORMULARI_DILUTION: '/formulari/dilution',
  FORMULARI_ANTIMICROBIAL: '/formulari/antimicrobial',
  FORMULARI_QUOTA: '/formulari/quota',
  FORMULARI_ALTERNATIVES: '/formulari/alternatives',
  HUB_PORTER: '/hub/porter',
  PORTER: '/porter',
  PORTER_DASHBOARD: '/porter/dashboard',
  PORTER_REQUEST_NEW: '/porter/requests/new',
  PORTER_MY_REQUESTS: '/porter/requests/my',
  PORTER_PANEL: '/porter/panel',
  PORTER_RECEIVER: '/porter/receiver',
  PORTER_MANAGER: '/porter/manager',
  PORTER_ROSTER: '/porter/roster',
  PORTER_REPORTS: '/porter/reports',
  PORTER_ROLES: '/porter/roles',
  HUB_TRANSPORTER: '/hub/transporter',
  TRANSPORTER_DASHBOARD: '/transporter/dashboard',
  TRANSPORTER_REQUEST_NEW: '/transporter/requests/new',
  TRANSPORTER_MY_REQUESTS: '/transporter/requests/my',
  TRANSPORTER_DRIVER_PANEL: '/transporter/driver/panel',
  TRANSPORTER_DRIVER_MONITOR: '/transporter/driver/monitor',
  TRANSPORTER_ADMIN_APPROVAL: '/transporter/admin/approval',
  TRANSPORTER_VEHICLE_REGISTRY: '/transporter/admin/vehicles',
  TRANSPORTER_VEHICLE_MOVEMENT: '/transporter/admin/vehicles/movement',
  TRANSPORTER_VEHICLE_ISSUES: '/transporter/admin/vehicles/issues',
  TRANSPORTER_REPORTS: '/transporter/admin/reports',
  TRANSPORTER_ROLES: '/transporter/admin/roles',

  HUB_PRIVILEDGING: '/priviledging',
  PRIVILEDGING: '/priviledging',
  PRIVILEDGING_DASHBOARD: '/priviledging',
  PRIVILEDGING_CRITERIA: '/priviledging/criteria',
  PRIVILEDGING_CATALOG: '/priviledging/catalog',
  PRIVILEDGING_MY_SUBMISSIONS: '/priviledging/my-submissions',
  PRIVILEDGING_REVIEW_QUEUE: '/priviledging/review-queue',
  PRIVILEDGING_STAFF_PROGRESS: '/priviledging/staff-progress',
  PRIVILEDGING_PRINT: '/priviledging/print',
  HUB_TEMPAHAN: '/tempahan',
  TEMPAHAN: '/tempahan',
  TEMPAHAN_DASHBOARD: '/tempahan',
  TEMPAHAN_CALENDAR: '/tempahan/kalendar',
  TEMPAHAN_REQUEST_NEW: '/tempahan/permohonan-baru',
  TEMPAHAN_MY_BOOKINGS: '/tempahan/permohonan-saya',
  TEMPAHAN_APPROVAL_QUEUE: '/tempahan/kelulusan',
  TEMPAHAN_ROOM_REGISTRY: '/tempahan/senarai-bilik',
  TEMPAHAN_REPORTS: '/tempahan/laporan',
  HUB_PERHIMPUNAN: '/hub/perhimpunan',
  HUB_KUNCI: '/hub/kunci',
  KUNCI_DASHBOARD: '/kunci/dashboard',
  KUNCI_DAFTAR: '/kunci/daftar',
  KUNCI_LOG: '/kunci/log',
  KUNCI_AUDIT: '/kunci/audit',
  KUNCI_POLISI: '/kunci/polisi',
  CROSSBORDER_DASHBOARD: '/crossborder/dashboard',
  CROSSBORDER_CREATE: '/crossborder/create',
  CROSSBORDER_DETAIL: '/crossborder/detail/:id',
  CROSSBORDER_LOG: '/crossborder/log',
  HUB_STAFF: '/hub/staff',
  HUB_CUTI: '/hub/staff',
  STAFF_DASHBOARD: '/staff/dashboard',
  STAFF_CALENDAR: '/staff/calendar',
  STAFF_MOVEMENT: '/staff/movement',
  STAFF_LEAVE: '/staff/leave',
  STAFF_LEAVE_APPLY: '/staff/leave/apply',
  STAFF_REMINDERS: '/staff/reminders',
  STAFF_DEADLINES: '/staff/deadlines',
  STAFF_HOD: '/staff/hod',
  STAFF_ORG_CHART: '/staff/org-chart',

  HUB_TIMEOFF: '/hub/time-off',
  HUB_MYPHIS: '/hub/myphis',
  HUB_MYMSDS: '/hub/mymsds',
  RESET_PASSWORD: '/reset-password',
  PROFILE: '/profile',
  ADMIN: '/admin',
  ADMIN_USERS: '/admin/users',
  ADMIN_USER_SESSIONS: '/admin/users/sessions',
  ADMIN_USER_ACTIVITY: '/admin/users/activity',
  ADMIN_ACCESS_REQUESTS: '/admin/access-requests',
  ADMIN_HOSPITALS: '/admin/hospitals',
  ADMIN_CLINICS: '/admin/clinics',
  ADMIN_DEPARTMENTS: '/admin/departments',
  ADMIN_ROLES: '/admin/roles',
  ADMIN_PERMISSIONS: '/admin/permissions',
  ADMIN_SETTINGS: '/admin/settings',
  ADMIN_AUDIT_LOGS: '/admin/audit-logs',
  ADMIN_MODULES: '/admin/modules',
  ADMIN_MONITORING: '/admin/monitoring',
  ADMIN_BACKUPS: '/admin/backups',
  ADMIN_ALERTS: '/admin/alerts',
  ADMIN_SYSTEM_LOGS: '/admin/system-logs',
  ADMIN_MEMOS: '/admin/memos',
  ADMIN_MEMO_CREATE: '/admin/memos/create',
  ADMIN_SENSITIVE_DATA_REQUESTS: '/admin/sensitive-data-requests',
  ADMIN_PATIENT_DATABASE: '/admin/patient-database',
  ADMIN_PATIENT_ACCESS_LOGS: '/admin/patient-database/access-logs',
  ADMIN_HOSPITAL_MODULES: '/admin/hospital-modules',
  ADMIN_REPORTS: '/admin/reports',
  ADMIN_REPORTS_USERS: '/admin/reports/users',
  ADMIN_REPORTS_ACCESS: '/admin/reports/access',
  ADMIN_REPORTS_USAGE: '/admin/reports/usage',
  ADMIN_REPORTS_SECURITY: '/admin/reports/security',
  ADMIN_HOSPITAL_HEALTH: '/admin/hospital-health',
  ADMIN_HOSPITAL_LOGS: '/admin/hospital-logs',
  ADMIN_HOSPITAL_BACKUPS: '/admin/hospital-backups',
  PHARMACY: '/pharmacy',
  PHARMACY_DASHBOARD: '/pharmacy/dashboard',
  PHARMACY_INVENTORY: '/pharmacy/inventory',
  PHARMACY_DRUGS: '/pharmacy/inventory/drugs',
  PHARMACY_NON_DRUGS: '/pharmacy/inventory/non-drugs',
  PHARMACY_FACILITY_DRUGS: '/pharmacy/inventory/facility-drugs',
  PHARMACY_FACILITY_NON_DRUGS: '/pharmacy/inventory/facility-non-drugs',
  PHARMACY_APPL_CATALOG: '/pharmacy/inventory/appl',
  PHARMACY_BUFFER_LEVEL: '/pharmacy/inventory/buffer-level',
  PHARMACY_ITEM_MOVEMENT: '/pharmacy/inventory/movement',
  PHARMACY_SLOW_MOVING: '/pharmacy/inventory/slow-moving',
  PHARMACY_NEAR_EXPIRY: '/pharmacy/inventory/near-expiry',
  PHARMACY_INVENTORY_REPORT: '/pharmacy/inventory/report',
  PHARMACY_BAD_STOCK: '/pharmacy/inventory/bad-stock',
  PHARMACY_LEDGER: '/pharmacy/inventory/ledger',
  PHARMACY_STORE_LOCATIONS: '/pharmacy/inventory/locations',
  PHARMACY_OXYGEN: '/pharmacy/oxygen',
  PHARMACY_OXYGEN_CYLINDERS: '/pharmacy/oxygen/cylinders',
  PHARMACY_OXYGEN_CONSUMPTION: '/pharmacy/oxygen/consumption',
  PHARMACY_OXYGEN_REPORTS: '/pharmacy/oxygen/reports',
  PHARMACY_OXYGEN_MAINTENANCE: '/pharmacy/oxygen/maintenance',
  PHARMACY_OXYGEN_LEDGER: '/pharmacy/oxygen/ledger',
  MYWARRANT_DASHBOARD: '/pharmacy/dashboard',
  PHARMACY_FINANCIAL: '/pharmacy/financial',
  PHARMACY_BUDGET: '/pharmacy/financial/budget',
  PHARMACY_WARRANT: '/pharmacy/financial/warrant',
  PHARMACY_APPL_ALLOCATION: '/pharmacy/financial/appl-allocation',
  PHARMACY_CC_ALLOCATION: '/pharmacy/financial/cc-allocation',
  PHARMACY_LP_ALLOCATION: '/pharmacy/financial/lp-allocation',
  PHARMACY_FORECAST: '/pharmacy/financial/forecast',
  PHARMACY_APPL: '/pharmacy/financial/appl',
  PHARMACY_CCDP: '/pharmacy/financial/ccdp',
  PHARMACY_PROCUREMENT: '/pharmacy/procurement',
  PHARMACY_PO: '/pharmacy/procurement/orders',
  PHARMACY_PO_CREATE: '/pharmacy/procurement/orders/create',
  PHARMACY_PO_DETAIL: '/pharmacy/procurement/orders/:id',
  PHARMACY_LPO: '/pharmacy/procurement/lpo',
  PHARMACY_LPO_CREATE: '/pharmacy/procurement/lpo/create',
  PHARMACY_DELIVERY: '/pharmacy/procurement/delivery',
  PHARMACY_RECEIVING: '/pharmacy/procurement/receiving',
  PHARMACY_PAYMENT: '/pharmacy/procurement/payment',
  PHARMACY_CREDIT_NOTE: '/pharmacy/procurement/credit-notes',
  PHARMACY_ORDER_TRACKING: '/pharmacy/procurement/tracking',
  PHARMACY_PENALTY: '/pharmacy/procurement/penalty',
  PHARMACY_PENALTY_DETAIL: '/pharmacy/procurement/penalty/:id',
  PHARMACY_LOU: '/pharmacy/procurement/lou',
  PHARMACY_SUPPLIER_PERFORMANCE: '/pharmacy/procurement/supplier-performance',
  PHARMACY_DISTRIBUTION: '/pharmacy/distribution',
  PHARMACY_INTER_FACILITY: '/pharmacy/distribution/inter-facility',
  PHARMACY_INTRA_FACILITY: '/pharmacy/distribution/intra-facility',
  PHARMACY_TRANSFER_REQUEST: '/pharmacy/distribution/transfer-request',
  PHARMACY_DISTRIBUTION_INDENT: '/pharmacy/inventory/distribution/indent',
  PHARMACY_DISTRIBUTION_ISSUE: '/pharmacy/inventory/distribution/issue',
  PHARMACY_DISTRIBUTION_ENTITLEMENT: '/pharmacy/inventory/distribution/entitlement',
  PHARMACY_CATALOG: '/pharmacy/catalog',
  PHARMACY_DRUG_CATALOG: '/pharmacy/catalog/drugs',
  PHARMACY_NON_DRUG_CATALOG: '/pharmacy/catalog/non-drugs',
  PHARMACY_SUPPLIER_CATALOG: '/pharmacy/catalog/suppliers',
  PHARMACY_CONTRACT_CATALOG: '/pharmacy/catalog/contracts',
  PHARMACY_HOSPITAL_FACILITY: '/pharmacy/catalog/hospitals',
  PHARMACY_CLINIC_FACILITY: '/pharmacy/catalog/clinics',
  PHARMACY_FACILITY_CATALOG: '/pharmacy/catalog/facilities',
  PHARMACY_MAINTENANCE: '/pharmacy/maintenance',
  PHARMACY_UNIT_CATALOG: '/pharmacy/maintenance/units',
  PHARMACY_STOCK_LOCATION: '/pharmacy/maintenance/locations',
  PHARMACY_STOCK_VERIFICATION: '/pharmacy/maintenance/verification',
  PHARMACY_REPORTS: '/pharmacy/reports',
  PHARMACY_REPORTS_INVENTORY: '/pharmacy/reports/inventory',
  PHARMACY_REPORTS_PROCUREMENT: '/pharmacy/reports/procurement',
  PHARMACY_REPORTS_FINANCIAL: '/pharmacy/reports/financial',
  PHARMACY_REPORTS_DISTRIBUTION: '/pharmacy/reports/distribution',
  PHARMACY_LOGS: '/pharmacy/logs',
  PHARMACY_PRODUCTS: '/pharmacy/products',
  PHARMACY_CATEGORIES: '/pharmacy/categories',
  PHARMACY_SUPPLIERS: '/pharmacy/suppliers',
  PHARMACY_PR: '/pharmacy/requisitions',
  PHARMACY_GR: '/pharmacy/receipts',
  PRIVACY_POLICY: '/privacy-policy',
  TERMS_OF_SERVICE: '/terms-of-service',
  UNAUTHORIZED: '/unauthorized',
} as const
