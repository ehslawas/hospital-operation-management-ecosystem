...
export default function GoodsReceivingForm({
  ...
  const initializeItems = (poData: any, prefill?: any) => {
    ...
    const initialItems: GoodsReceiptItemCreate[] = poData.items.map((item: any) => {
      ...
      const prefilledItem = prefill?.items?.find(
        (pi: any) => pi.poItemId === item.id
      )
      ...
      if (prefilledItem) {
        return {
          ...
          quantity_received: prefilledItem.quantity_received,
          quantity_accepted: prefilledItem.quantity_accepted,
          quantity_rejected: prefilledItem.quantity_rejected,
          disposition: 'accepted',
          rejection_reason: '',
          notes: 'Auto-populated from Excel',
          batches: prefilledItem.batches && prefilledItem.batches.length > 0
      ...
      const defaultQty = prefill ? 0 : qtyRemaining
      const isArrived = prefill ? false : qtyRemaining > 0
  ...
  const loadPo = async () => {
    ...
    if (res.error) {
      ...
      setPo(res.data)
      initializeItems(res.data, prefillData)
...
Matched Items: ${JSON.stringify((prefillData.items || []).map((i: any) => ({ name: i.item_name, code: i.item_code, batch: i.batch_number, exp: i.expiry_date, qty: i.quantity_received })), null, 2)}`)
  ...
  const formContent = (
    <div className={cn("flex flex-col bg-white", isEmbedded ? "h-full" : "max-h-[96vh]")}>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                REFERENCE (LPO): <span className="text-blue-400">{po?.lpo?.[0]?.lpo_number || po?.po_number || 'LOADING...'}</span>
              </span>
              {po?.lpo?.[0]?.lpo_number && (
                <>
                  <div className="w-1 h-1 bg-slate-700 rounded-full" />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    PO REF: <span className="text-slate-200">{po.po_number}</span>
                  </span>
                </>
              )}
            </div>
        ...
        {isLoading ? (
          ...
          <>
            {po.status === 'completed' ? (
              ...
              <>
                {error && (
                  <div className="bg-rose-900 border-l-4 border-rose-500 p-6 flex items-start gap-4 mb-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
                  {/* Left Column: manifest and comments */}
                  <div className="lg:col-span-4 space-y-4">
                    {prefillData && (
                      <div className="space-y-2">
                        <div className="bg-emerald-900 border-l-4 border-emerald-500 p-3 rounded-lg flex items-center justify-between gap-3 text-white">
                          <div className="flex items-center gap-3">
                            <IconCheckCircle className="w-5 h-5 text-emerald-400 shrink-0" />
                            <div>
                              <h4 className="text-xs font-black uppercase tracking-widest leading-none">Excel Sync Pre-fill Active</h4>
                              <p className="text-[10px] text-emerald-250 font-bold uppercase tracking-tight mt-1.5">
                                Matched items, quantities, batch numbers, and expiry dates have been auto-populated from your spreadsheet.
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                      <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-2">
                        ...
                        <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">OFFICIAL INTAKE MANIFEST</h3>
                      </div>
                      <div className="p-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2.5">
                        <div className="space-y-1">
                          ...
                          <Input 
                            ...
                            className={cn(
                              "rounded-md border-slate-200 h-8 font-bold text-xs",
                        ...
                        <div className="space-y-1">
                          <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Delivery Order No. (DO) *</label>
                          <Input 
                            type="text" 
                            placeholder="MANDATORY FOR REGISTRY"
                            value={deliveryNote} 
                            onChange={(e) => setDeliveryNote(e.target.value)} 
                            className="rounded-md border-slate-200 h-8 font-bold text-xs placeholder:text-rose-300"
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Scheduled Date</label>
                          <Input 
                            type="date"
                            value={scheduledDate}
                            onChange={(e) => setScheduledDate(e.target.value)}
                            className="rounded-md border-slate-200 h-8 font-bold text-xs"
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Delivery Order Documents (DO)</label>
                          <div className="space-y-2">
                            <div className="relative">
                              ...
                              <label
                                htmlFor="do-upload"
                                className={cn(
                                  "flex items-center gap-2.5 w-full h-8 px-3 border-2 border-dashed rounded-md text-[10px] font-black transition-all cursor-pointer uppercase tracking-widest",
                            ...
                            {documentUrls.length > 0 && (
                              <div className="space-y-1">
                                {documentUrls.map((url, index) => {
                                  ...
                                  return (
                                    <div key={index} className="flex items-center justify-between px-2.5 py-1.5 bg-emerald-50 border border-emerald-100 rounded-md group animate-in fade-in slide-in-from-left-1 duration-200">
                        ...
                        <div className="space-y-1">
                          <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Invoice Identifier</label>
                          <Input 
                            type="text" 
                            placeholder="REQUIRED FOR FINANCE"
                            value={invoiceNumber} 
                            onChange={(e) => setInvoiceNumber(e.target.value)} 
                            className="rounded-md border-slate-200 h-8 font-bold text-xs placeholder:text-slate-300"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <label className="block text-[10px] font-black text-slate-900 uppercase tracking-widest">AUDIT COMMENTS & REMARKS</label>
                      </div>
                      <textarea 
                        className="w-full min-h-[350px] p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold placeholder:text-slate-300 focus:bg-white focus:border-slate-900 transition-all outline-none"
                  ...
                  {/* Right Column: Line items and log */}
                  <div className="lg:col-span-8 space-y-4">
                    {/* Line Items Registry */}
                    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                      <div className="px-4 py-2.5 bg-slate-900 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col lg:flex-row gap-3 justify-between items-start">
                                  ...
                                  <div className="flex items-start gap-3">
                                    <div className="pt-0.5">
                                      <input 
                                        ...
                                        className="disabled:opacity-40 disabled:cursor-not-allowed w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 transition-all cursor-pointer accent-indigo-600"
                                      />
                                    </div>
                                    <div className="space-y-0.5">
                                      ...
                                      <h4 
                                        className={cn(
                                          "text-xs font-black text-slate-900 leading-tight uppercase italic select-none",
                                  ...
                                  {isArrived && (
                                    <div className="grid grid-cols-2 gap-2 pl-7">
                                      <div className="bg-slate-50 border border-slate-200 px-2 py-1 rounded-md">
                                        <p className="text-[8px] font-black text-slate-400 uppercase mb-0.5">TOTAL ORDER</p>
                                        <p className="text-xs font-black text-slate-900">{item.quantity_ordered}</p>
                                      </div>
                                      <div className="bg-blue-50 border border-blue-100 px-2 py-1 rounded-md">
                                        <p className="text-[8px] font-black text-blue-400 uppercase mb-0.5">PENDING</p>
                                        <p className="text-xs font-black text-blue-700">{remaining}</p>
                                      </div>
                                    </div>
                                  )}
                                </div>
                                ...
                                {isArrived && remaining > 0 ? (
                                  <div className="lg:w-3/5 grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-1">
                                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Rec. Qty</label>
                                      <Input 
                                        type="number" 
                                        min="0"
                                        value={item.quantity_received} 
                                        onChange={(e) => handleItemChange(idx, 'quantity_received', parseInt(e.target.value, 10))} 
                                        className="rounded-md h-8 font-black text-center text-sm"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest text-rose-500">Rej. Qty</label>
                                      <Input 
                                        type="number" 
                                        min="0"
                                        value={item.quantity_rejected} 
                                        onChange={(e) => handleItemChange(idx, 'quantity_rejected', parseInt(e.target.value, 10))} 
                                        className="rounded-md h-8 font-black text-center text-sm text-rose-600 border-rose-200 bg-rose-50/30"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest text-emerald-500">Accepted</label>
                                      <div className="h-8 bg-emerald-500 text-white rounded-md flex items-center justify-center text-sm font-black shadow-md shadow-emerald-500/10">
                                        {item.quantity_accepted}
                                      </div>
                                    </div>
                                  </div>
                                ) : (
                                  <div className="lg:w-3/5 flex items-center justify-between bg-slate-50 border border-dashed border-slate-200 rounded px-2.5 py-1 select-none animate-in fade-in zoom-in-95 duration-200 self-center">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                      {remaining === 0 ? "Fulfillment Complete" : "Item Excluded"}
                                    </span>
                                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                                      {remaining === 0 
                                        ? "Fully received in previous shipment" 
                              ...
                              {isArrived && (
                                <div className="mt-4 space-y-2">
                                  <div className="flex items-center justify-between border-b border-slate-100 pb-1">
                                    ...
                                    <button onClick={() => handleAddBatch(idx)} className="text-[9px] font-black text-white bg-slate-900 px-2 py-0.5 rounded uppercase tracking-tighter hover:bg-slate-800 transition-all flex items-center gap-1 shadow-sm animate-in fade-in">
                                  ...
                                  <div className="space-y-1.5">
                                    {item.batches.map((batch, bIdx) => (
                                      <div key={bIdx} className="grid grid-cols-1 md:grid-cols-12 gap-2 items-end bg-slate-50 p-2.5 rounded-md border border-slate-200/60 group hover:bg-white hover:border-slate-400 transition-all">
                                        <div className="md:col-span-3">
                                          <label className="block text-[9px] font-black text-slate-400 uppercase mb-0.5">Batch Registry #</label>
                                          <Input value={batch.batch_number} onChange={(e) => handleBatchChange(idx, bIdx, 'batch_number', e.target.value)} className="h-8 text-xs font-bold uppercase rounded border-slate-200" />
                                        </div>
                                        <div className="md:col-span-3">
                                          <label className="block text-[9px] font-black text-slate-400 uppercase mb-0.5">Mfg Date</label>
                                          <Input type="date" value={batch.manufacturing_date || ''} onChange={(e) => handleBatchChange(idx, bIdx, 'manufacturing_date', e.target.value)} className="h-8 text-xs font-bold rounded border-slate-200" />
                                        </div>
                                        <div className="md:col-span-3">
                                          <label className="block text-[9px] font-black text-slate-400 uppercase mb-0.5">Expiry Date *</label>
                                          <Input type="date" value={batch.expiry_date} onChange={(e) => handleBatchChange(idx, bIdx, 'expiry_date', e.target.value)} className="h-8 text-xs font-bold rounded border-slate-200" />
                                        </div>
                                        <div className="md:col-span-2">
                                          <label className="block text-[9px] font-black text-slate-400 uppercase mb-0.5">Registry Qty</label>
                                          <Input type="number" value={batch.quantity || ''} onChange={(e) => handleBatchChange(idx, bIdx, 'quantity', e.target.value)} className="h-8 text-xs font-black text-center rounded border-slate-200" />
                                        </div>
                                  ...
                                  <div className={cn(
                                    "mt-2.5 p-3 rounded-md border transition-all",
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      />
                                      <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest italic cursor-pointer">ISSUE CREDIT NOTE FOR REMAINING DEFICIT</label>
                                    </div>
                                    {item.mark_remaining_as_credit_note && (
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3 pl-6 animate-in slide-in-from-top-2 duration-300">
                                        <div className="space-y-1">
                                          <label className="block text-[9px] font-black text-amber-600 uppercase tracking-widest">DEFICIT QUANTITY</label>
                                          <Input type="number" value={item.credit_note_quantity || ''} onChange={(e) => handleItemChange(idx, 'credit_note_quantity', parseInt(e.target.value, 10))} className="h-8 border-amber-200 bg-white font-black" />
                                        </div>
                                        <div className="space-y-1">
                                          <label className="block text-[9px] font-black text-amber-600 uppercase tracking-widest">FORMAL REASON</label>
                                          <Input type="text" placeholder="REQUIRED FOR CREDIT NOTE" value={item.credit_note_reason || ''} onChange={(e) => handleItemChange(idx, 'credit_note_reason', e.target.value)} className="h-8 border-amber-200 bg-white font-bold" />
                                        </div>
                                      </div>
                                    )}
                                  </div>
      ...
      {po && po.status !== 'completed' && (
        <div className="px-6 py-4 border-t border-slate-200 bg-white sticky bottom-0 z-30 shadow-[0_-15px_30px_rgba(0,0,0,0.03)] flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-slate-100 rounded flex items-center justify-center text-slate-400">
              <IconShieldCheck className="w-4 h-4" />
            </div>
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">REGISTRY STATUS</p>
              <p className="text-[11px] font-black text-slate-900 uppercase italic mt-0.5">AWAITING AUTHORIZATION</p>
            </div>
          </div>
          <div className="flex gap-3">
            {!isEmbedded && (
              <button 
                onClick={onClose}
                className="px-6 py-2.5 bg-white border border-slate-200 rounded-md text-[10px] font-black text-slate-500 uppercase tracking-widest hover:border-slate-900 hover:text-slate-900 transition-all"
              >
                DISCARD
              </button>
            )}
            <button
              ...
              className="px-8 py-2.5 bg-slate-900 text-white rounded-md font-black text-[11px] uppercase tracking-[0.3em] shadow-lg shadow-slate-900/20 hover:bg-slate-800 hover:-translate-y-0.5 active:scale-95 transition-all flex items-center gap-2.5"
            >
  ...
  return (
    <>
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[240] animate-in fade-in duration-300" onClick={onClose} />
      <div className="fixed inset-0 z-[250] flex items-center justify-center p-2">
        <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl max-h-[96vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
          {formContent}
        </div>
      </div>
    </>
