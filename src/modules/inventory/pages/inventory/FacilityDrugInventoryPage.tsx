...
import {
  ...
  batchAddToFacilityDrugInventory,
  updateFacilityDrugInventoryItem,
  removeFromFacilityDrugInventory,
  clearFacilityDrugInventoryByScheme,
} from '@/services/pharmacy/facilityDrugInventoryService'
...
export const FacilityDrugInventoryPage: React.FC = () => {
  ...
  const [catalogDrugs, setCatalogDrugs] = useState<DrugWithRelations[]>([])
  const [categories, setCategories] = useState<DrugCategory[]>([])
  const [isCatalogLoading, setIsCatalogLoading] = useState(false)

  // Facility-specific inventory items
  const [facilityItems, setFacilityItems] = useState<FacilityDrugItem[]>([])

  // Modal & Search states
  const [isAddModalOpen, setIsAddModalOpen] = useState(false)
  const [modalSearch, setModalSearch] = useState('')
  const [modalVoteFilter, setModalVoteFilter] = useState<string>('all')
  const [selectedCatalogDrug, setSelectedCatalogDrug] = useState<DrugWithRelations | null>(null)
  ...
  // Edit Drawer state
  const [editingItem, setEditingItem] = useState<FacilityDrugItem | null>(null)
  ...
  useEffect(() => {
    ...
    loadFacilityDrugInventory(hospitalId).then(items => {
      if (isMounted) {
  ...
  const handleAddItem = async () => {
    ...
    const res = await addToFacilityDrugInventory(
      hospitalId, selectedCatalogDrug,
    ...
    const fresh = await loadFacilityDrugInventory(hospitalId)
    setFacilityItems(fresh)
    showSuccess(`'${selectedCatalogDrug.drug_name}' berjaya ditambah ke Inventori Fasiliti.`)
  ...
  const handleBatchAddItems = async () => {
    if (selectedCatalogDrugIds.length === 0) return
    const drugsToAdd = catalogDrugs.filter(d => selectedCatalogDrugIds.includes(d.id))
    if (drugsToAdd.length === 0) return
    const res = await batchAddToFacilityDrugInventory(
    ...
    const fresh = await loadFacilityDrugInventory(hospitalId)
    setFacilityItems(fresh)
    showSuccess(`${res.added} item ubat berjaya ditambah ke Inventori Fasiliti.${ res.skipped > 0 ? ` (${res.skipped} sudah wujud, dilangkau.)` : '' }`)
    setSelectedCatalogDrugIds([])
    setSelectedCatalogDrug(null)
    setIsAddModalOpen(false)
  }
  ...
  const handleDeleteItem = async (drugId: string, drugName: string, e: React.MouseEvent) => {
    e.stopPropagation()
    await removeFromFacilityDrugInventory(hospitalId, drugId)
    const fresh = await loadFacilityDrugInventory(hospitalId)
    setFacilityItems(fresh)
  ...
  const handleRemoveItem = async (id: string, name: string) => {
    ...
    await removeFromFacilityDrugInventory(hospitalId, id)
    const fresh = await loadFacilityDrugInventory(hospitalId)
    setFacilityItems(fresh)
  ...
  const handleClearCurrentScheme = async () => {
    ...
    await clearFacilityDrugInventoryByScheme(hospitalId, currentVoteParam)
    const fresh = await loadFacilityDrugInventory(hospitalId)
    setFacilityItems(fresh)
  ...
  // Open Edit Drawer
  const handleOpenEditModal = (drug: FacilityDrugItem) => {
    setEditingItem(drug)
    ...
    setEditTherapeuticCat(getDrugTherapeuticCategory(drug))
    setEditPrescriberCat(getDrugPrescriberCategory(drug))
    ...
    async function loadRealData() {
      try {
        ...
        if (isSupabaseConfigured()) {
          ...
          if (editingItem.drug_code && editingItem.drug_code !== 'N/A') {
            poConditions.push(`item_code.eq.${editingItem.drug_code}`)
          }
        ...
        try {
          ...
          if (localTxStr) {
            ...
            const matchedLocalTx = localTxArr.filter((t: any) =>
              ...
              t.item_code === editingItem.drug_code ||
              (t.drug_name && editingItem.drug_name && t.drug_name.toLowerCase() === editingItem.drug_name.toLowerCase())
            )
          ...
          if (localPoStr) {
            ...
            const matchedLocalPOs = localPoArr.filter((po: any) =>
              po.items?.some((i: any) => i.drug_id === editingItem.id || i.item_code === editingItem.drug_code)
            )
  ...
  useEffect(() => {
    ...
    const code = editingItem.drug_code || editingItem.item_code || editingItem.sku || editingItem.id
    const payload = JSON.stringify({
      code,
      name: editingItem.drug_name || editingItem.item_name,
      type: 'DRUG',
  ...
  const handlePrintQrLabel = () => {
    ...
    const code = editingItem.drug_code || editingItem.item_code || editingItem.sku || '-'
    const name = editingItem.drug_name || editingItem.item_name
    ...
    const uom = editingItem.dosage_form || editingItem.uom || editingItem.unit_of_measure || 'PACK'
  ...
  const handleSaveEdit = async () => {
    ...
    const result = await updateFacilityDrugInventoryItem(hospitalId, editingItem.id, {
    ...
    const fresh = await loadFacilityDrugInventory(hospitalId)
    setFacilityItems(fresh)
    showSuccess(`Konfigurasi stok untuk '${editingItem.drug_name}' berjaya dikemaskini.`)
    setEditingItem(null)
  }
  ...
  const categoryOptions = useMemo(() => {
    ...
    facilityItems.forEach(item => {
      const cat = getDrugTherapeuticCategory(item)
  ...
  const filteredItems = useMemo(() => {
    return facilityItems.filter(item => {
      ...
      if (search) {
        ...
        const therapeuticCategory = getDrugTherapeuticCategory(item).toLowerCase()
        const prescriberCat = getDrugPrescriberCategory(item).toLowerCase()
        const matchCode = code.toLowerCase().includes(q)
        const matchName = item.drug_name?.toLowerCase().includes(q)
      ...
      if (categoryId) {
        const itemCat = getDrugTherapeuticCategory(item).toLowerCase()
      ...
      if (prescriberFilter && prescriberFilter !== 'all') {
        const itemPrescriber = getDrugPrescriberCategory(item)
      ...
      const nameA = a.drug_name || a.item_name || ''
      const nameB = b.drug_name || b.item_name || ''
  ...
  }, [catalogDrugs, facilityItems, modalVoteFilter, modalSearch])

  // Select all toggle helper
  const allFilteredIds = useMemo(() => filteredCatalogDrugs.map(d => d.id), [filteredCatalogDrugs])
  const isAllSelected = useMemo(() => {
    return allFilteredIds.length > 0 && allFilteredIds.every(id => selectedCatalogDrugIds.includes(id))
  }, [allFilteredIds, selectedCatalogDrugIds])
  ...
  return (
    <div className="p-6 space-y-6 w-full animate-fadeIn">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 p-6 rounded-2xl text-white shadow-xl border border-blue-800/40 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-1">
          ...
          <p className="text-slate-300 text-sm max-w-2xl">
            Pilih dan tambah item ubat daripada Katolog Utama ke inventori fasiliti anda secara manual.
          </p>
        </div>
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          >
            ...
            {isExporting ? 'Menjana PDF...' : 'Eksport Formulari PDF'}
          </button>

          <Button
            size="md"
            onClick={() => setIsAddModalOpen(true)}
            className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold shadow-lg transition-all rounded-xl flex items-center gap-2 px-5 py-2.5"
          >
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
        {[
          ...
          <button
            key={tab.id}
            onClick={() => setSearchParams(tab.id === 'all' ? {} : { vote: tab.id })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all">
          ...
          <p className="text-xs text-slate-500 mt-1">Sedia dipreksripsikan</p>
        </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200/80 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            >
              <option value="" className="text-slate-800 bg-white font-medium">Semua Kategori Ubat</option>
            ...
            <select
              ...
              onChange={e => setPrescriberFilter(e.target.value)}
              className="px-3 py-2 text-sm rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 font-medium cursor-pointer"
            >
              <option value="all" className="text-slate-800 bg-white font-medium">Semua Preskriber (FUKKM)</option>
              <option value="A*" className="text-slate-800 bg-white font-medium">Kategori A* (Pakar Khusus)</option>
              <option value="A" className="text-slate-800 bg-white font-medium">Kategori A (Pakar)</option>
              <option value="A/KK" className="text-slate-800 bg-white font-medium">Kategori A/KK (Pakar / FMS KK)</option>
              <option value="B" className="text-slate-800 bg-white font-medium">Kategori B (Pegawai Perubatan)</option>
              <option value="C" className="text-slate-800 bg-white font-medium">Kategori C (Paramedik / MA / Nurse)</option>
            </select>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200">
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-slate-50 text-xs uppercase font-semibold text-slate-500 border-b border-slate-200">
              <tr>
                <th className="py-3.5 px-4">Nama & Kod Ubat</th>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {paginatedItems.length === 0 ? (
                  ...
                  <td colSpan={10} className="py-16 text-center">
                    <div className="max-w-md mx-auto space-y-3">
                      ...
                      <p className="text-xs text-slate-500">
                        Inventori ubat fasiliti anda masih kosong {currentVoteParam !== 'all' ? `untuk skim ${currentVoteParam.toUpperCase()}` : ''}. Sila klik butang di bawah untuk memilih item ubat daripada Katolog Utama.
                      </p>
                      <Button
                        size="sm"
                        onClick={() => setIsAddModalOpen(true)}
                ...
                paginatedItems.map(drug => {
                  ...
                  const therapeuticCat = getDrugTherapeuticCategory(drug)
                  const prescriberCat = getDrugPrescriberCategory(drug)
                  ...
                  return (
                    ...
                    >
                      <td className="py-3 px-4">
                        ...
                        <div className="font-semibold text-slate-900">{drug.drug_name}</div>
                        {drug.generic_name && (
                          <div className="text-xs text-slate-400 italic">{drug.generic_name}</div>
                        )}
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        <div className="font-semibold text-slate-900 uppercase">
                          {drug.dosage_form || drug.uom || 'PACK'}
                        </div>
                      ...
                      <td className="py-3 px-4 text-center">
                        <button
                          onClick={(e) => handleDeleteItem(drug.id, drug.drug_name, e)}
          ...
          <div className="fixed inset-y-0 right-0 w-full sm:w-[540px] md:w-[680px] lg:w-[760px] bg-white shadow-2xl z-[100] flex flex-col border-l border-slate-200 animate-in slide-in-from-right duration-300">
            ...
            <div className="p-6 bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white flex items-center justify-between shadow-md">
              <div>
                ...
                <h2 className="text-xl font-bold tracking-tight">Pilih Item Ubat Daripada Katolog</h2>
                <p className="text-xs text-slate-300 mt-0.5">
                  Pilih satu atau beberapa item ubat untuk dimasukkan ke dalam pegangan inventori fasiliti anda.
                </p>
              </div>
              <button
                onClick={() => {
              ...
              {!selectedCatalogDrug ? (
                <>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200/80 shadow-sm">
                    <div className="relative flex-1">
                      ...
                      <input
                        type="text"
                        placeholder="Cari kod barang, nama ubat atau bahan aktif..."
                        value={modalSearch}
                  ...
                  <div className="flex items-center justify-between px-1 py-1">
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                      Senarai Item Katolog ({filteredCatalogDrugs.length} dijumpai)
                    </div>
                  ...
                  <div className="space-y-2">
                    {isCatalogLoading ? (
                      <div className="py-16 text-center text-slate-400">
                        ...
                        <p className="text-sm font-medium text-slate-600">Memuatkan Katolog Ubat Utama...</p>
                      </div>
                    ) : filteredCatalogDrugs.length === 0 ? (
                      ...
                      <div className="divide-y divide-slate-100 border border-slate-200/80 rounded-2xl bg-white overflow-hidden shadow-sm">
                        {filteredCatalogDrugs.map(drug => {
                          ...
                          const priceVal = drug.price ?? drug.unit_price ?? 0
                          const isSelected = selectedCatalogDrugIds.includes(drug.id)
                            ...
                            >
                              <div className="flex items-start gap-3 flex-1 pr-4">
                                <input
                                  type="checkbox"
                                  checked={isSelected}
                                  onChange={e => handleToggleSelectDrug(drug.id, e)}
                                ...
                                >
                                  ...
                                  <div className="text-xs text-slate-400">
                                    Bentuk: <span className="font-medium text-slate-600 uppercase">{drug.dosage_form || drug.uom || 'PACK'}</span> • Harga: <span className="font-mono font-semibold text-slate-700">RM {priceVal.toFixed(2)}</span>
                                  </div>
                                </div>
                              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50/70 p-5 rounded-2xl border border-blue-100 space-y-2">
                    ...
                    <h3 className="font-bold text-slate-900 text-xl">{selectedCatalogDrug.drug_name}</h3>
                    {selectedCatalogDrug.generic_name && (
                      <p className="text-xs text-slate-500 italic">{selectedCatalogDrug.generic_name}</p>
                    )}
                    <div className="flex items-center gap-3 flex-wrap pt-2">
                      <span className="font-mono text-xs font-bold px-2.5 py-1 rounded-lg bg-white text-blue-700 border border-blue-200">
                        {selectedCatalogDrug.drug_code || selectedCatalogDrug.item_code || selectedCatalogDrug.sku || '-'}
                      </span>
                      <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 uppercase">
                        {(selectedCatalogDrug.procurement_vote || 'APPL').toUpperCase()}
                      </span>
                      {(selectedCatalogDrug.cc_contract_number || selectedCatalogDrug.kkm_contract_number || selectedCatalogDrug.contract_number) && (
                        <span className="font-mono text-xs font-bold px-2.5 py-1 rounded-lg bg-amber-50 text-amber-800 border border-amber-200">
                          No. Kontrak: {selectedCatalogDrug.cc_contract_number || selectedCatalogDrug.kkm_contract_number || selectedCatalogDrug.contract_number}
                        </span>
                      )}
                      {renderContractPeriodBadge(
                        selectedCatalogDrug.cc_contract_start_date || (selectedCatalogDrug as any).contract_start_date,
                        selectedCatalogDrug.cc_contract_end_date || (selectedCatalogDrug as any).contract_end_date,
                        selectedCatalogDrug.cc_contract_status,
                        selectedCatalogDrug.procurement_vote,
                        selectedCatalogDrug.cc_contract_number || selectedCatalogDrug.kkm_contract_number || selectedCatalogDrug.contract_number
                      )}
                      <span className="text-xs text-slate-600 font-mono">
                        Harga: RM {((selectedCatalogDrug.price ?? selectedCatalogDrug.unit_price ?? 0)).toFixed(2)}
                      </span>
                    </div>
                  </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
              ...
              {selectedCatalogDrug ? (
              ...
              ) : selectedCatalogDrugIds.length > 0 ? (
                <Button
                  size="md"
                ...
                >
                  <Plus className="w-4 h-4 mr-1.5" />
                  Tambah {selectedCatalogDrugIds.length} Item ke Inventori
                </Button>
              ) : null}
            </div>
          </div>
              ...
              <div>
                <div className="flex items-center gap-2 text-xs font-semibold text-blue-300 uppercase tracking-wider mb-1">
                  <span className="font-mono bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded border border-blue-400/30">
                    {editingItem.drug_code || editingItem.item_code || editingItem.sku || '-'}
                  </span>
                  <span className="text-emerald-400 font-bold">• STOK FASILITI</span>
                </div>
                <h2 className="text-xl font-bold tracking-tight text-white">{editingItem.drug_name}</h2>
                ...
                <>
                  ...
                  <div className="bg-amber-50/70 p-4 rounded-2xl border border-amber-200/80 space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-800 uppercase">
                          {editingItem.dosage_form || editingItem.uom || 'PACK'}
                        </span>
                      </div>
                      <div>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-slate-700 block mb-1">
                          Kategori Ubat (Therapeutic Class)
                        </label>
                        <select
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="B">B (Pegawai Perubatan)</option>
                    ...
                    <div className="space-y-2 text-xs">
                      {isTxLoading ? (
                        ...
                        <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 text-center text-slate-400 text-xs font-medium">
                          Tiada rekod pesanan pembelian (PO) ditemui dalam pangkalan data untuk ubat ini.
                        </div>
                      ) : (
                        realPOs.map(po => (
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                    ...
                    <div className="space-y-2 text-xs">
                      {isTxLoading ? (
                        ...
                        <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 text-center text-slate-400 text-xs font-medium">
                          Tiada rekod penerimaan stok (GRN) ditemui dalam pangkalan data untuk ubat ini.
                        </div>
                      ) : (
                        realTransactions
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="space-y-1 overflow-hidden">
                            ...
                            <div className="font-bold text-xs text-slate-900 line-clamp-2 leading-tight">
                              {editingItem.drug_name}
                            </div>
                            <div className="text-[10px] text-slate-500">
                              Packaging: <span className="font-semibold text-slate-700 uppercase">{editingItem.dosage_form || editingItem.uom || 'PACK'}</span>
                            </div>
                      ...
                      <div className="space-y-4 max-w-xs text-center md:text-left">
                        ...
                        <div className="flex flex-col gap-2 pt-2">
                          ...
                          {qrCodeUrl && (
                            <a
                              href={qrCodeUrl}
                              download={`QR-${editingItem.drug_code || editingItem.id}.png`}
...
export default FacilityDrugInventoryPage
