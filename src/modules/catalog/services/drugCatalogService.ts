...
import { mockDrugs, mockDrugCategories, mockSuppliers } from '@/services/pharmacy/mockData'
...
export async function getDrugCatalog(
  ...
  try {
    if (isSupabaseConfigured()) {
      ...
      // Order alphabetically by drug_name (then by code), so list naturally shows A â†’ Z
      query = query
...
export async function batchImportDrugs(
  ...
  try {
      ...
      if (existing) {
        ...
        try {
          ...
          if (updateResult.error) {
            ...
            successCount++
            console.log(`[batchImportDrugs] âœ“ Updated drug: ${drugCode}`)
          }
        } catch (error) {
        ...
        try {
          ...
          if (createResult.error) {
            ...
            successCount++
            console.log(`[batchImportDrugs] âœ“ Created drug: ${drugCode}`)
          }
        } catch (error) {
    ...
    if (supabaseConfigured) {
      ...
      console.warn('[batchImportDrugs] âš  WARNING: Items saved to mock data, not Supabase!')
    }
    console.log('='.repeat(60))
