...
import { mockNonDrugs, mockNonDrugCategories, mockSuppliers } from '@/services/pharmacy/mockData'
...
export async function getNonDrugCatalog(
  ...
  try {
    ...
    if (supabaseConfigured) {
      ...
      // Order alphabetically by item_name (then by code) so list shows A â†’ Z
      query = query
...
export async function createNonDrug(
  ...
  try {
    ...
    const insertData: any = {
      ...
      // â€œinvalid input syntax for type uuidâ€ errors when Excel
    ...
    if (supabaseConfigured) {
      console.log('[createNonDrug] âœ“ Supabase is configured, attempting insert...')
      try {
        ...
        if (error) {
          console.error('[createNonDrug] âœ— Supabase INSERT ERROR:', error)
        ...
        if (!data) {
          console.error('[createNonDrug] âœ— No data returned from Supabase insert')
          return {
            data: null,
        ...
        console.log('[createNonDrug] âœ“ Successfully inserted into Supabase. ID:', data.id)
        ...
        console.error('[createNonDrug] âœ— Exception during Supabase insert:', insertError)
        return {
          data: null,
      ...
      console.warn('[createNonDrug] âš  Supabase not configured - using mock data')
    ...
    console.log('[createNonDrug] âš  WARNING: Item was saved to mock data, not Supabase!')
    console.log('[createNonDrug] ========== END (MOCK) ==========')

    return { data: nonDrugWithRelations, error: null }
  } catch (error) {
    console.error('[createNonDrug] âœ— EXCEPTION in createNonDrug:', error)
