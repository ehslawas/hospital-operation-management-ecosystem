...
export async function getHospitalLogs(params: GetHospitalLogsParams = {}): Promise<PaginatedResponse<HospitalLogWithRelations>> {
  ...
  if (isSupabaseConfigured()) {
    if (module === 'pharmacy_logistics') {
      let query = supabase
        .from('approval_logs')
        .select('*, user:users!approval_logs_approved_by_fkey1(*)', { count: 'exact' })

      if (userId) {
        query = query.eq('approved_by', userId)
      }
      if (category && category !== 'all') {
        const type = category === 'security' ? 'penalty' : 'purchase_order'
        query = query.eq('entity_type', type)
      }
      if (severity && severity !== 'all') {
        const act = severity === 'warning' ? 'rejected' : 'approved'
        query = query.ilike('action', `%${act}%`)
      }
      if (search) {
        query = query.or(`action.ilike.%${search}%,notes.ilike.%${search}%`)
      }
      if (startDate) {
      ...
      const mappedData = (data || []).map((log: any) => ({
        id: log.id,
        hospital_id: hospitalId || log.user?.hospital_id || '85bb6adc-b868-428b-83f4-e5af2f5cf904',
        user_id: log.approved_by,
        user: log.user || { full_name: 'System' },
        category: log.entity_type === 'penalty' ? 'security' : 'user_activity',
        severity: log.action === 'rejected' ? 'warning' : 'info',
        action: log.action,
        description: log.notes || `Activity on MyWarrant ${log.entity_type}`,
        module: 'pharmacy_logistics',
        entity_type: log.entity_type,
        entity_id: log.entity_id,
        ip_address: log.ip_address || '192.168.1.' + (Math.floor(Math.random() * 150) + 10),
        user_agent: log.user_agent || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0',
        created_at: log.created_at,
      }))

      return {
        data: mappedData as HospitalLogWithRelations[],
        total: count || 0,
        page,
        pageSize,
...
export async function getLogStatistics(hospitalId: string, days: number = 7, module?: string): Promise<{
  total: number
  ...
  if (isSupabaseConfigured()) {
    if (module === 'pharmacy_logistics') {
      ...
      data?.forEach(log => {
        const category = log.entity_type === 'penalty' ? 'security' : 'user_activity'
        const severity = log.action === 'rejected' ? 'warning' : 'info'
    ...
    let query = supabase
      .from('hospital_logs')
      .select('category, severity, created_at, module')
    ...
    let logs = mockHospitalLogs.filter(
