// src/modules/mytransporter/services/transporterCrossborderPdf.ts
// PDF Generation Service for Merged Crossborder Transporter requests using jsPDF
 ...
 */
export async function generateCrossborderTransferFormPDF(request: TransportRequest): Promise<Blob> {
  ...
  // Travel dates and time extraction
  let dateTravel = '';
  let timeDeparture = '';
  if (request.tarikh_masa_diperlukan) {
    const dt = new Date(request.tarikh_masa_diperlukan);
    dateTravel = dt.toISOString().split('T')[0];
    timeDeparture = String(dt.getHours()).padStart(2, '0') + ':' + String(dt.getMinutes()).padStart(2, '0');
  }

  const cbData = request.crossborder_data;

  // Trip details table
  const tripData = [
    ['Referring Hospital:', cbData?.tempat_berlepas || 'Hospital Lawas'],
    ['Date of Travel:', getFormattedDate(dateTravel)],
    ['Time of Departure:', timeDeparture],
    ['Place of Departure:', cbData?.tempat_berlepas || 'Hospital Lawas']
  ];
  ...
  const patients = cbData?.patients || [];
  ...
  const registrationNo = request.kenderaan?.no_kenderaan || 'BNN 7608';
  const otherEquip = request.mesin_diperlukan && request.mesin_diperlukan.length > 0 
    ? request.mesin_diperlukan.join(', ') 
    : 'N/A';

  const vehicleBody = [
    ['REGISTRATION NO:', registrationNo],
    ['ANY OTHER EQUIPMENT:', otherEquip]
  ];
  ...
  const driverBody = [
    ['DRIVER NAME:', cbData?.pemandu_nama || 'N/A'],
    ['PASSPORT NO:', cbData?.pemandu_passport || 'N/A']
  ];
  ...
  const warisEscorts = cbData?.waris_escorts || [];
  const escortTableData = warisEscorts.length > 0 
    ? warisEscorts.map((e, idx) => [idx + 1, e.nama, e.jenis_dokumen, e.no_dokumen, e.hubungan || ''])
  ...
  const kkmEscorts = cbData?.kkm_escorts || [];
  const medicalTableData = kkmEscorts.length > 0
    ? kkmEscorts.map((e, idx) => [idx + 1, e.nama, e.jenis_dokumen, e.no_dokumen, e.jawatan || ''])
  ...
  doc.setFontSize(9);
  doc.text(`Name: ${cbData?.doktor_perujuk_nama || request.nama_pemohon || 'Dr. Jason Ling'}`, margin, y + 3.5);
 ...
 */
export async function generateCrossborderPermissionLetterPDF(request: TransportRequest): Promise<Blob> {
  const doc = new jsPDF('p', 'mm', 'a4');
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  ...
  let dateTravel = '';
  if (request.tarikh_masa_diperlukan) {
    dateTravel = request.tarikh_masa_diperlukan.split('T')[0];
  }

  const cbData = request.crossborder_data;

  // Metadata block (Date and Ref) right-aligned (Official government style)
  const refNo = cbData?.surat_kebenaran_ref || `TF/HL/MW(32)${new Date().getFullYear()}`;
  doc.setFont('times', 'normal');
  doc.setFontSize(10.5);
  doc.text(`Rujukan Kami : ${refNo}`, pageWidth - margin, y, { align: 'right' });
  doc.text(`Tarikh : ${getFormattedDate(dateTravel)}`, pageWidth - margin, y + 6, { align: 'right' });
  ...
  doc.setFont('times', 'normal');
  doc.setFontSize(10.5);
  const bodyText = `With reference to the above, I would like to request permission for the following patient to cross the border. This patient is to be referred to ${request.destinasi || 'HOSPITAL LIMBANG'} for further management.`;
  ...
  doc.line(margin, y, margin + 60, y);
  doc.setFont('times', 'bold');
  const directorName = cbData?.pengarah_nama ? cbData.pengarah_nama.split('(')[0].trim() : 'DR. DOUGLAS CHU KIN SOON';
