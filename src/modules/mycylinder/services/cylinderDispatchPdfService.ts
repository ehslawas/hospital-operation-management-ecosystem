...
export async function generateCylinderDispatchPdf(
  ...
  if (request.approver && request.status !== 'pending' && request.status !== 'cancelled') {
    const appDateStr = request.approved_date ? new Date(request.approved_date).toLocaleDateString('ms-MY') : 'â€”';
  ...
  if (request.issuer && (request.status === 'issued' || request.status === 'completed')) {
    const issueDateStr = request.issued_date ? new Date(request.issued_date).toLocaleDateString('ms-MY') : 'â€”';
