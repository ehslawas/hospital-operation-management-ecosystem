...
export async function getSystemStatistics(): Promise<ApiResponse<SystemStatistics>> {
  try {
    if (isSupabaseConfigured()) {
      ...
      const uniqueHospitals = new Set(enabledModules.map((m) => m.hospital_id))
      ...
      // Supabase is required for System Admin functionality
      return {
        data: null,
        error: 'Supabase is not configured. System Admin features require database connection.',
      }
    }
  } catch (error) {
