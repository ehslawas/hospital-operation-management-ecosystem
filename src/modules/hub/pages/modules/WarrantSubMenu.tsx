...
export const WarrantSubMenu: React.FC = () => {
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900">MyWarrant</h1>
            <p className="text-slate-500">{language === 'ms' ? 'Pengurusan Waran Kewangan' : 'Financial Warrant Management'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
        ...
        <button 
          onClick={() => navigate(ROUTES.MYWARRANT_DASHBOARD)}
          className="mt-6 px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors"
        >
          {language === 'ms' ? 'Pergi ke Dashboard Waran' : 'Go to Warrant Dashboard'}
        </button>
      </div>
    </div>
  )
